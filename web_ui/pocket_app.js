...
if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.hostname.startsWith('192.168.')) {
    // Usa a mesma porta em que o site foi servido (ex: 8080)
...
class PocketDirectorApp {
    ...
    setInterval(() => {
      ...
      if (isSpeakingNow) {
        if (!this.isSpeakingVAD) {
          ...
          // Barge-in: Cancela o áudio do TTS tocando atualmente e avisa o servidor para cancelar a geração vLLM
          if (this.currentAudioElement) {
              this.currentAudioElement.pause();
              this.currentAudioElement = null;
          }
          this.isPlayingAudio = false;
          this.audioQueue = []; // Limpa a fila
          
          if (this.socket && this.socket.readyState === WebSocket.OPEN) {
              this.socket.send(JSON.stringify({ type: "vad_interrupt" }));
          }
  ...
  handleVAD(energy) {
    ...
    if (energy > threshold) {
      ...
      if (this.isPlayingAudio && !this.bargeInTriggered) {
        ...
        this.sendToColmeia(JSON.stringify({ type: 'vad_interrupt' }));
        this.triggerHaptic('bargein');
      }
  ...
  connectWebSocket() {
    ...
    this.ws.onmessage = (event) => {
      if (typeof event.data === 'string') {
        ...
        // Play audio chunk from backend sequentially (Chat Ao Vivo)
  ...
  playNextAudio() {
      if (this.audioQueue.length === 0) {
          this.isPlayingAudio = false;
          this.currentAudioElement = null;
          this.setOrbState('online'); // Volta ao normal quando acaba de falar
          return;
      }
