...
document.addEventListener('DOMContentLoaded', () => {
    ...
    toggleUpscale.addEventListener('change', () => applyUpscaleStyleFull(toggleUpscale.checked));
        ...
        async function autoTagLyrics() {
            ...
            btn.disabled = true;
            try {
                const response = await fetch("http://163.176.135.59/api/music/auto_tag_lyrics", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
        ...
        async function generateBatchIdeas() {
            ...
            try {
                const response = await fetch("http://163.176.135.59/api/music/generate_batch_ideas", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
        ...
        async function generateMusicMaster() {
            ...
            window.batchGeneratedFiles = window.batchGeneratedFiles || [];
                        document.getElementById('previewPlaceholder').style.display = 'none';
            const preview = document.getElementById('previewContainer');
            preview.style.display = 'block';
            
            if (!document.getElementById('batchTracksList')) {
                preview.innerHTML = `
            ...
            for (let i = 0; i < prompts.length; i++) {
                ...
                if (durationMode === 'fixed') {
                  ...
                  logMusicMaster("[" + (i+1) + "/" + prompts.length + "] Solicitando geração na nuvem (Conta 9)...");
                  if (prompts.length > 1) updateMusicProgress(i, prompts.length);
                  
                  try {
                      const response = await fetch("http://163.176.135.59/api/audio/generate", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                      ...
                      let initResult;
                      try {
                          initResult = JSON.parse(textResponse);
                      } catch (e) {
                          throw new Error(`Oráculo retornou erro 500: ${textResponse.substring(0, 100)}`);
                      }
                      
                      let result = initResult;
                      if (initResult.status === "queued") {
                          const jobId = initResult.job_id;
                          logMusicMaster("[" + (i+1) + "/" + prompts.length + "] Job iniciado [" + jobId + "]. Aguardando...");
                          
                          while (true) {
                              await new Promise(r => setTimeout(r, 5000)); // Poll a cada 5s
                              
                              const pollRes = await fetch("http://163.176.135.59/api/audio/status/" + jobId);
                              ...
                              if (pollData.status === "success") {
                                  ...
                                  throw new Error(pollData.error || "Erro na geração.");
                              }
                              logMusicMaster("[" + (i+1) + "/" + prompts.length + "] Processando na nuvem... [" + jobId + "]");
                          }
                      }
                      ...
                      if (result.success || result.status === "success") {
                          ...
                          successCount++;
                          const fileUrlAbs = result.file_url.startsWith('http') ? result.file_url : `http://163.176.135.59${result.file_url}`;
                          window.batchGeneratedFiles.push(fileUrlAbs);
                        ...
                        const trackHtml = `
                            <div style="background: #1e1e28; padding: 15px; border-radius: 8px; border: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; gap: 15px;">
                                <div style="flex: 1;">
                                    <h4 style="color: var(--cyan); margin-top:0; margin-bottom: 10px;">Faixa ${index}: ${p.substring(0,30)}...</h4>
                                    <audio controls style="width: 100%;">
                                        <source src="${fileUrlAbs}" type="audio/mpeg">
                                    </audio>
                                </div>
                                <a href="${fileUrlAbs}" download class="btn btn-primary" style="white-space: nowrap; height: fit-content;">⬇️ Baixar</a>
                            </div>
                        `;
                        trackList.insertAdjacentHTML('beforeend', trackHtml);

                        if (window.apolloTransferOS) { 
                            window.apolloTransferOS.addItem("audio", result.file_url.split('/').pop(), "Música: " + p.substring(0,20), null, { url: fileUrlAbs }); 
                        }
                    } else {
            ...
            if (window.batchGeneratedFiles && window.batchGeneratedFiles.length > 0) {
                document.getElementById('batchDownloadAllContainer').style.display = 'inline-block';
                document.getElementById('batchDownloadAllContainer').innerHTML = '⬇️ Baixar Todas (' + window.batchGeneratedFiles.length + ')';
            }
        }
