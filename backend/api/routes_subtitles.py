...
# Simula banco de dados em mem�ria para as tarefas (jobs) de legendas
subtitle_jobs = {}
...
def process_subtitle_job(job_id: str, req: SubtitleRequest):
    ...
    try:
        ...
        # O modelo do vosk por padr�o tenta buscar no diret�rio ou usar um pr�-configurado
        # Aqui, como estamos focados em rodar, passamos um caminho padr�o ou vazio
        vosk_model_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'pre_edicao', 'vosk-model-small-pt-0.3')
        
        # Chama a fun��o extra�da das antigas ferramentas locais do Tkinter
        ...
        logger.info(f"[{job_id}] Legendas conclu�das. V�deo salvo em {final_video}")
...
async def generate_subtitles(request: SubtitleRequest, background_tasks: BackgroundTasks):
    """
    Endpoint para gerar legendas locais usando Vosk/Whisper e embutir no v�deo com FFmpeg.
    """
    if not os.path.exists(request.media_path):
        raise HTTPException(status_code=400, detail="M�dia de entrada n�o encontrada localmente.")
    ...
    return {
        "status": "success",
        "job_id": job_id,
        "message": "Tarefa de gera��o de legendas iniciada localmente."
    }
