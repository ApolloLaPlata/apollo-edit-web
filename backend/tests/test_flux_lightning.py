...
# Configure Lightning AI API Key (from previous tests)
LIGHTNING_API_KEY = "sk-lit-8d641291-d92e-4469-8465-4a74d1c28a5d"
MODAL_FLUX_URL = "https://historiasde7dias--apollo-render-router-apollo-api.modal.run/generate/image"
...
async def orchestrate_multi_character_generation(base_scenario: str, characters: list[dict]):
...
Rewrite the full prompt to include this new character seamlessly in a specific position (e.g., left, right, foreground).
        """
        try:
    ...
    # Strict resolutions for FLUX to avoid anatomical distortion
    payload = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("status") == "success" and data.get("image_base64"):
                ...
                output_path = Path(os.path.dirname(__file__)) / "flux_multichar_test.jpg"
...
if __name__ == "__main__":
    scenario = "A epic confrontation inside a futuristic neon cyberpunk diner at night."
    chars = [
        {"name": "Cyber-Ninja", "details": "A male ninja wearing a sleek black mechanical suit with glowing blue accents and a neon katana."},
        {"name": "Hologram Waitress", "details": "A translucent pink holographic female waitress wearing a 1950s diner uniform."}
    ]
    asyncio.run(orchestrate_multi_character_generation(scenario, chars))
