...
(function() {
    function injectAssistant() {
        ...
        chatPanel.innerHTML = `
            ...
            <div id="assistant-chat-history" style="flex: 1; padding: 16px; overflow-y: auto; color: #e2e8f0; font-size: 14px; display: flex; flex-direction: column; gap: 12px;">
                <div style="background: #1e293b; padding: 12px; border-radius: 8px 8px 8px 2px; align-self: flex-start; max-width: 85%; border: 1px solid rgba(255,255,255,0.05); line-height: 1.5;">
                    Ol! Eu sou o Assistente Apollo. Como posso auxiliar em sua edio ou roteirizao hoje?
                </div>
            </div>
        ...
        // O Boto Flutuante (Assistente Apollo)
        ...
        const sendMessage = async () => {
            ...
            try {
                ...
                if (apiKeysRef && aiRef) {
                    ...
                    const toolKB = {
                        "editor_texto.html": "Voc est no Editor de Texto Mgico. O usurio pode escrever roteiros. Diga a ele para usar a barra superior para formatar ou salvar na Garagem.",
                        "editor_imagem.html": "Voc est no Photopea (Clone do Photoshop). Ensine atalhos do Photoshop, fale sobre camadas (layers), mscaras e filtros.",
                        "editor_audiomass.html": "Voc est no AudioMass. Diga ao usurio que ele pode arrastar udios, cortar partes, aplicar efeitos (reverb, compresso) no topo.",
                        "editor_daw.html": "Voc est na DAW Profissional (BandLab/GarageBand style). Ensine sobre multipistas, arranjo, MIDI e plugins.",
                        "gerador_musica.html": "Voc est no Gerador de Msica IA. Diga ao usurio para inserir um prompt (ex: 'Bossa nova triste com saxofone') e gerar.",
                        "removedor_fundo.html": "Voc est no Removedor de Fundo. Diga para o usurio colar ou upar uma imagem para extrair o fundo com IA.",
                        "estudio_dublagem.html": "Voc est no Estdio de Dublagem (RVC). O usurio pode upar um udio original, escolher um modelo de voz (Personagem) e a IA vai clonar o timbre.",
                        "storyboard.html": "Voc est no Mapa Mental (Excalidraw). Ensine que o usurio pode desenhar ns, caixas e setas para planejar vdeos.",
                        "teleprompter.html": "Voc est no Teleprompter. Ensine o usurio a colar o texto, ajustar a velocidade e gravar a si mesmo lendo a tela.",
                        "editor_freecut.html": "Voc est no Editor de Vdeo (Timeline). Ensine a arrastar clipes, cortar (Split), exportar o projeto, e usar transies.",
                        "tts.html": "Voc est no Motor Voz IA (TTS). Ensine a colar roteiros, ajustar o Pitch e Speed e clicar em Gerar udio.",
                        "ferramenta_laplata.html": "Voc est na La Plata IA. Ajude o usurio a interagir com os ns neurais profundos do Apollo.",
                        "hub.html": "Voc est no Hub Central Apollo. Ensine o usurio a clicar nas ferramentas ao redor para entrar nos editores."
                    };
                    
                    const contextInfo = toolKB[currentPage] || "Voc est no Apollo Edit Web.";
                    
                    const navMap = `
Mapeamento de Navegao Vlido (Use com a tag [NAVIGATE: arquivo.html]):
- editor_texto.html (Editor de Texto / Roteiros)
- editor_imagem.html (Editor de Imagem / Photopea)
- editor_audiomass.html (Edio de udio / AudioMass)
- editor_daw.html (DAW de Msica Avanada)
- gerador_musica.html (Gerador de Msica IA)
...
- teleprompter.html (Teleprompter para gravao)
- timeline.html (Editor de Vdeo)
- tts.html (Motor de Voz TTS)
- hub.html (Tela Inicial Central)`;

                    const promptText = `O usurio diz: ${text}`;
                    ...
                    try {
                        ...
                        if (activeId) {
                            ...
                            if (ch) {
                                channelContext = `\n\n[CONTEXTO GLOBAL DO CANAL ATUAL]\nNome do Canal: ${ch.name}\nDescrio do Canal: ${ch.description || 'No informada'}\nSua Personalidade/Tom de Voz para este canal: ${ch.personality || 'Aja profissionalmente.'}\nLinks de Pesquisa/Referncias do Canal: ${ch.links && ch.links.length > 0 ? ch.links.map(l => l.title + ' - ' + l.url).join(', ') : 'Nenhum'}`;
                            }
                        }
                    } catch(e) {}

                    const systemMsg = `Voc  o Assistente Apollo, uma IA poderosa, prestativa e integrada ao ecossistema Apollo OS.${channelContext}\n\n[CONTEXTO DA FERRAMENTA ATUAL ONDE O USURIO EST]\n${contextInfo}\n\n${navMap}\n\nHABILIDADE DE NAVEGAO:\nSe o usurio pedir para ir para outra ferramenta, voc DEVE usar a tag oculta [NAVIGATE: arquivo.html] no final da sua resposta consultando o mapa acima. Exemplo: "Entendido, abrindo o editor de udio. [NAVIGATE: editor_audiomass.html]".\n\nSiga estritamente a "Sua Personalidade/Tom de Voz" se fornecida no contexto do canal.`;
                    ...
                    throw new Error("Laplata API no carregada no escopo global.");
                }
            } catch (err) {
                console.error(err);
                history.removeChild(loadingMsg);
                ...
                errorMsg.textContent = 'Erro de conexo. O subsistema de I.A. no est disponvel no momento.';
    ...
    // Tentar injetar assim que possvel
