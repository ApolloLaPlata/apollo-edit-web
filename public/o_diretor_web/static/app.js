...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sel) {
        sel.addEventListener('change', () => {
            ...
            previewBox.innerHTML = `<img src="${url}" style="max-width: 100%; max-height: 100%; object-fit: contain;" onerror="this.parentElement.innerHTML='<span style=\\'color:#ef4444;font-size:0.8rem;\\'> Sem print salvo.</span>'">`;
        });
    }
        ...
        btnDelete.addEventListener('click', async () => {
            ...
            if (confirm(`Tem certeza que deseja EXCLUIR o perfil '${name}' permanentemente do seu sistema?`)) {
                try {
                    ...
                    if (r.ok) {
                        alert(`Perfil '${name}' excludo com sucesso!`);
                        previewBox.innerHTML = '<span style="color: #475569; font-size: 0.85rem;">Sem preview...</span>';
                        loadProfileList(); // Recarrega a lista
                    } else {
                        alert("Erro ao excluir perfil. Ele pode no existir mais.");
                    }
                } catch(e) {
