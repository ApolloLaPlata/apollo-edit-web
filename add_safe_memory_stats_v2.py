...
new_block = '''    mock_dir = "/tmp/mock_cuda"
...
for f in files:
    ...
    pattern = r'    mock_dir = "/tmp/mock_cuda"[\s\S]+?os\.environ\.pop\("PYTHONPATH", None\)'
    
    content, count = re.subn(pattern, new_block, content)
    print(f"Updated force_cpu_during_snapshot in {f} ({count} matches)")
