...
function syncFromSidebar(id) {
    ...
    layersData[id].x = parseInt(document.getElementById(`x_${id}`).value) || 0;
    layersData[id].y = parseInt(document.getElementById(`y_${id}`).value) || 0;
    layersData[id].w = parseInt(document.getElementById(`w_${id}`).value) || 100;
    layersData[id].h = parseInt(document.getElementById(`h_${id}`).value) || 100;
    updateBox(l);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ao selecionar um perfil, exibe a imagem
    if (sel) {
        sel.addEventListener('change', () => {
            const name = sel.value;
            if (!name) {
                previewBox.innerHTML = '<span style="color: #475569; font-size: 0.85rem;">Sem preview...</span>';
                return;
            }
            // Adicionamos o tempo na url para evitar cache
            const url = `/api/preview_image?name=${name}&t=${new Date().getTime()}`;
            previewBox.innerHTML = `<img src="${url}" style="max-width: 100%; max-height: 100%; object-fit: contain;" onerror="this.parentElement.innerHTML='<span style=\\'color:#ef4444;font-size:0.8rem;\\'>⚠️ Sem print salvo.</span>'">`;
        });
    }
    ...
    if (btnDelete) {
        btnDelete.addEventListener('click', async () => {
            ...
            if (confirm(`Tem certeza que deseja EXCLUIR o perfil '${name}' permanentemente do seu sistema?`)) {
                try {
                    const r = await fetch(`/api/delete_profile?name=${name}`);
                    if (r.ok) {
                        alert(`Perfil '${name}' excluído com sucesso!`);
                        previewBox.innerHTML = '<span style="color: #475569; font-size: 0.85rem;">Sem preview...</span>';
                        loadProfileList(); // Recarrega a lista
                    } else {
                        alert("Erro ao excluir perfil. Ele pode não existir mais.");
                    }
                } catch(e) {
