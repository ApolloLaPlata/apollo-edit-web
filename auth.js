...
// ATENÃ‡ÃƒO: Substitua pelos dados do seu projeto Supabase
...
async function checkAuthStatus() {
    if (!supabase) return { id: 'dev-user', email: 'comandante@v5apollo.com' };
...
async function requireAuth() {
    ...
    if (!user) {
        // Se nÃ£o tiver logado e nÃ£o estiver na pÃ¡gina de login, redireciona.
        if (!window.location.pathname.includes('login.html')) {
            // window.location.href = 'login.html';
        }
        return { id: 'dev-user', email: 'comandante@v5apollo.com' };
    } else {
        // Atualiza a interface (Avatar, Email, CrÃ©ditos) se os elementos existirem
...
async function loadWebUserProfile() {
    try {
        ...
        // FASE 4: Bloqueios e Globais (Banimento e ManutenÃ§Ã£o)
        if (data.is_banned) {
            document.body.innerHTML = `
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100vh; background:#000; color:#fff; text-align:center; padding:20px; z-index:999999999; position:fixed; top:0; left:0; width:100vw;">
                    ...
                    <p style="font-size:1.2rem; color:#aaa; max-width:500px;">Sua conta foi suspensa por violaÃ§Ã£o dos termos de uso ou pendÃªncia administrativa. Entre em contato com o suporte para mais informaÃ§Ãµes.</p>
                </div>
            `;
            return true;
        }
        ...
        if (data.module_settings) {
            if (data.module_settings.maintenance_mode === 'on' && !data.is_master) {
                document.body.innerHTML = `
                    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100vh; background:#111; color:#fff; text-align:center; padding:20px; z-index:999999999; position:fixed; top:0; left:0; width:100vw;">
                        <h1 style="color:#facc15; font-size:3rem; font-family:'Bangers', cursive; margin-bottom:10px;">ðŸ› ï¸ MANUTENÃ‡ÃƒO PROGRAMADA ðŸ› ï¸</h1>
                        <p style="font-size:1.2rem; color:#aaa; max-width:500px;">O Apollo Edit estÃ¡ passando por atualizaÃ§Ãµes. Voltaremos em breve!</p>
                    </div>
                `;
                return true;
            }
            ...
            if (data.module_settings.global_broadcast && data.module_settings.global_broadcast.trim().length > 0) {
                ...
                banner.innerHTML = `âš ï¸ <strong>AVISO GLOBAL:</strong> ${data.module_settings.global_broadcast} <span style="float:right; cursor:pointer;" onclick="this.parentElement.remove()">X</span>`;
                document.body.appendChild(banner);
            }
        }
        
        // 1. Atualizar crÃ©ditos (Gasolina) se existir o elemento no Header
        ...
        // Inicializar Cristais no localStorage para persistÃªncia local (mock shop)
        ...
        // Atualiza display de cristais na pÃ¡gina de perfil
        ...
        if (avatarContainer) {
            // Determinar raridade do Card ou CosmÃ©tico Equipado
            ...
            if (oldAvatar) {
                ...
                rpgCard.title = `${data.is_master ? 'MASTER' : (data.is_pro ? 'PRO' : 'FREE')} (NÃ­vel ${userLevel})`;
                ...
                rpgCard.innerHTML = `
                    ...
                    <div class="corner-badge mini hacker-badge ${data.is_hacker ? '' : 'locked'}" 
                         ...
                         id="hdr-hacker-badge"
                         title="${data.is_hacker ? 'Hacker Licenciado âš™ï¸' : 'Acesso Hacker Bloqueado ðŸ”’ - Clique p/ liberar'}">
                        ${data.is_hacker ? 'âš™ï¸' : 'ðŸ”’'}
                    </div>
                    <div class="corner-badge mini flag-badge" 
                         ...
                         title="PaÃ­s: ${data.pais === 'BR' || !data.pais ? 'Brasil' : data.pais}">
                        ðŸ‡§ðŸ‡·
                    </div>
        ...
        if (userNameEl) {
            ...
            if (data.is_hacker) {
                badgesHTML += `<span style="background: rgba(155, 89, 182, 0.2); color: #c084fc; border: 1px solid #9B59B6; font-size: 10px; padding: 2px 5px; border-radius: 4px; font-weight: bold; text-shadow: 0 0 5px rgba(155, 89, 182, 0.5); margin-left: 5px; display: inline-flex; align-items: center; gap: 2px;">âš™ï¸ HACKER</span>`;
            }
            if (data.is_pro) {
                badgesHTML += `<span style="background: rgba(251, 191, 36, 0.2); color: #fbbf24; border: 1px solid #fbbf24; font-size: 10px; padding: 2px 5px; border-radius: 4px; font-weight: bold; text-shadow: 0 0 5px rgba(251, 191, 36, 0.5); margin-left: 5px; display: inline-flex; align-items: center; gap: 2px;">ðŸ‘‘ PRO</span>`;
            }
            if (data.is_master) {
                badgesHTML += `<span style="background: rgba(6, 182, 212, 0.2); color: #22d3ee; border: 1px solid #06b6d4; font-size: 10px; padding: 2px 5px; border-radius: 4px; font-weight: bold; text-shadow: 0 0 5px rgba(6, 182, 212, 0.5); margin-left: 5px; display: inline-flex; align-items: center; gap: 2px;">âš¡ MASTER</span>`;
            }

            // Construir subtÃ­tulo com base no nÃ­vel/tipo
        ...
        // 4. Atualizar elementos da pÃ¡gina perfil.html se ela for a ativa
        ...
        if (avatarFrame) {
            ...
            avatarFrame.innerHTML = `
                ...
                <div class="corner-badge flag-badge" style="background:#000; border-radius:50%;" title="PaÃ­s: Brasil">
                    ðŸ‡§ðŸ‡·
                </div>
                <div class="corner-badge rank-badge" title="Rank Global: ${data.rank_global || '#1'}">
                    <span>${data.rank_global || '#1'}</span>
                </div>
                <div class="badge-edge-right" title="Upgrade de Tier / CustomizaÃ§Ãµes disponÃ­veis!" onclick="showSaaSPricingModal()">â–²</div>
            `;
        ...
        statRows.forEach(row => {
            if (row.innerHTML.includes('NÃVEL:')) {
        ...
        // FASE 37: Aplicar permissÃµes de mÃ³dulos com base no cargo (Role-based access)
        applyModulePermissions(data);

        return false; // Indica que nÃ£o foi bloqueado
    } catch (e) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="upsell-card">
                ...
                <div class="hacker-badge-stamp">âš¡ CURSO DE INFRA E AUTOMAÃ‡ÃƒO ðŸ’»</div>
                <p style="font-size:1.1rem; font-weight:800; text-shadow:1px 1px 0 #000; color:#fff; margin-bottom:15px;">
                    Quer rodar o Apollo Edit com <strong style="color:var(--btn-yellow);">CUSTO DE SERVIDOR R$ 0,00</strong> gerando Ã¡udio e vÃ­deo ilimitado?
                </p>
                <p style="font-size:0.95rem; color:#cbd5e1; font-weight:700;">
                    Ao adquirir nosso curso completo de R$ 400, vocÃª aprende a instalar e configurar o ecossistema local na sua mÃ¡quina com Pinokio, RVC de voz e conexÃµes BYOK!
                </p>
                
                <div class="feature-list">
                    <div class="feature-item">
                        <span class="feature-icon">âš™ï¸</span>
                        <span><strong>BYOK Inteligente:</strong> Insira suas prÃ³prias chaves e gaste $0 em servidores.</span>
                    </div>
                    <div class="feature-item">
                        <span class="feature-icon">ðŸŽ™ï¸</span>
                        <span><strong>Clonagem RVC local:</strong> Treinamento de voz e imitaÃ§Ã£o perfeita de timbres.</span>
                    </div>
                    <div class="feature-item">
                        <span class="feature-icon">ðŸ’»</span>
                        <span><strong>ConfiguraÃ§Ã£o Estendida:</strong> Desbloqueie roteamentos, chaves e conexÃµes de porta locais.</span>
                    </div>
                    <div class="feature-item">
                        <span class="feature-icon">ðŸ‘‘</span>
                ...
                <div class="price-tag-hacker">
                    R$ 400 <span style="font-size:1rem; color:#aaa;">(VitalÃ­cio Curso + 1 ano Status Badge)</span>
                    <div style="font-size:0.8rem; color:#64748b; margin-top:5px;">RenovaÃ§Ã£o da badge por R$ 200/ano</div>
                </div>
                
                <button class="btn yellow" style="width:100%; font-size:1.6rem; animation: pulse 2s infinite;" onclick="buyHackerCourse()">
                    âš¡ QUERO SER HACKER (ADQUIRIR AGORA)
                </button>
            </div>
        `;
...
function buyHackerCourse() {
    alert("ðŸš€ Redirecionando para o Checkout de R$ 400 (EducaÃ§Ã£o + Upgrade de Conta Hacker)!\n\nApÃ³s o pagamento, o sistema irÃ¡ liberar as ferramentas extensas de API BYOK, roteamento e seu selo Hacker estarÃ¡ ativo por 1 ano.");
            ...
            <div class="pricing-container">
                ...
                <div class="pricing-grid">
                    ...
                    <div class="pricing-card pro">
                        <h3 class="pricing-card-title">ðŸ‘‘ PLANO PRO</h3>
                        <div class="pricing-price">R$ 49 <span style="font-size:0.9rem; color:#aaa;">/ mÃªs</span></div>
                        
                        <div class="pricing-features">
                            <div class="pricing-feature-line">âœ¨ Borda Dourada Ã‰pica (Gold Frame)</div>
                            <div class="pricing-feature-line">â›½ 5.000 L de Gasolina de Render</div>
                            <div class="pricing-feature-line">ðŸ¤– Acesso total ao Diretor de IA V2</div>
                            <div class="pricing-feature-line">âš¡ Prioridade mÃ©dia na Fila de Render</div>
                        </div>
                    ...
                    <div class="pricing-card master">
                        ...
                        <h3 class="pricing-card-title">âš¡ PLANO MASTER</h3>
                        <div class="pricing-price">R$ 98 <span style="font-size:0.9rem; color:#aaa;">/ mÃªs</span></div>
                        
                        <div class="pricing-features">
                            <div class="pricing-feature-line">ðŸ’Ž Borda Neon Ciano LendÃ¡ria (Legendary)</div>
                            <div class="pricing-feature-line">â›½ 12.000 L de Gasolina de Render (O Dobro!)</div>
                            <div class="pricing-feature-line">ðŸŽï¸ Skins e Trajes LendÃ¡rios de VestiÃ¡rio</div>
                            <div class="pricing-feature-line">ðŸš€ Prioridade MÃ¡xima InstantÃ¢nea no Render</div>
                        </div>
...
// Verifica na inicializaÃ§Ã£o da pÃ¡gina
document.addEventListener('DOMContentLoaded', async () => {
    const currentPage = window.location.pathname.split('/').pop() || 'index.html';
    if(currentPage === 'admin.html' || currentPage === 'admin_login.html') return;

    // Carregar informaÃ§Ãµes do perfil unificado E checar bloqueios
    ...
    // FASE 34: InjeÃ§Ã£o do Master Toggle Button no HUD (Dev Tools)
    injectDevModeToggle();

    // FASE 36: Tintura TemÃ¡tica (Theme Tinting)
    applyChannelTheme();
});
...
async function checkPageAccessAndLog() {
    ...
    try {
        ...
        if (data.settings) {
            // Verifica permissÃ£o da pÃ¡gina inteira
            ...
            if (reqAccess !== 'Free') {
                ...
                if (userLevel < reqLevel) {
                    alert(`ACESSO NEGADO: Esta pÃ¡gina requer nÃ­vel ${reqAccess}.`);
...
async function renderAds() {
    try {
        ...
        if(data.success && data.ads.length > 0) {
            // Escolhe um AD aleatÃ³rio ativo
            const ad = data.ads[Math.floor(Math.random() * data.ads.length)];
            
            // Cria div de anÃºncio no canto inferior direito
...
function injectDevModeToggle() {
    ...
    toggleBtn.innerHTML = currentMode === 'cloud' ? 'â˜ï¸ Mudar p/ Local' : 'ðŸ’» Mudar p/ Nuvem';
