...
export async function POST(request: Request) {
  try {
    ...
    db.prepare(`UPDATE DaoTopic SET votes = votes + 1 WHERE id = ?`).run(topicId);

    // Checar se atingiu 100 votos
    const topic = db.prepare(`SELECT * FROM DaoTopic WHERE id = ?`).get(topicId) as any;
    
    if (topic && topic.votes >= 100 && topic.status === 'pending') {
       db.prepare(`UPDATE DaoTopic SET status = 'approved' WHERE id = ?`).run(topicId);
