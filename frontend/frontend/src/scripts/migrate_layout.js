...
try {
  ...
  db.prepare(`ALTER TABLE Blog ADD COLUMN primaryColor TEXT DEFAULT '#3b82f6'`).run();
...
try {
  db.prepare(`ALTER TABLE Blog ADD COLUMN secondaryColor TEXT DEFAULT '#1e40af'`).run();
...
try {
  db.prepare(`ALTER TABLE Blog ADD COLUMN layoutStyle TEXT DEFAULT 'magazine'`).run();
