...
export function Dashboard() {
  ...
  const networks = [
    { name: 'YouTube', icon: Youtube, color: 'text-red-500', bg: 'bg-red-900/30', followers: '450K', growth: '+2.1K', status: 'active' },
    { name: 'Instagram', icon: Instagram, color: 'text-pink-600', bg: 'bg-pink-50', followers: '320K', growth: '+1.5K', status: 'active' },
    { name: 'TikTok', icon: Video, color: 'text-black', bg: 'bg-zinc-800', followers: '280K', growth: '+4.2K', status: 'active' },
    { name: 'Kwai', icon: Video, color: 'text-orange-500', bg: 'bg-orange-900/30', followers: '110K', growth: '+800', status: 'active' },
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Monitoramento em tempo real de todas as suas redes sociais.</p>
        </div>
        
        <button className="flex items-center gap-2 px-4 py-2 bg-zinc-900 border border-zinc-800 text-zinc-700 rounded-lg hover:bg-transparent transition-colors shadow-sm text-sm font-medium">
          <RefreshCw size={16} />
          Sincronizar Dados
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className={`p-2 rounded-lg bg-indigo-900/30 text-indigo-600`}>
                <stat.icon size={20} />
              </div>
            ...
            <h3 className="text-3xl font-bold text-zinc-100 mb-1">{stat.value}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Networks Breakdown */}
        <div className="lg:col-span-2 bg-zinc-900 rounded-xl border border-zinc-800 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-zinc-100">
            <h3 className="text-lg font-semibold text-zinc-100">Desempenho por Rede</h3>
          </div>
          <div className="divide-y divide-zinc-100">
            {networks.map((network, i) => (
              <div key={i} className="p-4 sm:p-6 flex items-center justify-between hover:bg-transparent transition-colors">
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${network.bg} ${network.color}`}>
                    <network.icon size={24} />
                  </div>
                  <div>
                    <h4 className="font-semibold text-zinc-100">{network.name}</h4>
                    <p className="text-sm text-zinc-400">Última atualização há 5 min</p>
                  </div>
                </div>
                
                <div className="text-right">
                  <div className="font-bold text-zinc-100 text-lg">{network.followers}</div>
        ...
        <div className="space-y-6">
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 shadow-sm p-6">
            <h3 className="text-lg font-semibold text-zinc-100 mb-4">Alertas de Audiência</h3>
            <div className="space-y-4">
              <div className="p-4 rounded-lg bg-red-900/30 border border-red-100">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-400 mt-1">Seu último vídeo teve 40% menos alcance que a média. Considere mudar o formato.</p>
                  </div>
                </div>
              </div>
              
              <div className="p-4 rounded-lg bg-emerald-900/30 border border-emerald-100">
                <div className="flex items-start gap-3">
                  <TrendingUp className="text-emerald-600 mt-0.5" size={18} />
                  <div>
                    <h4 className="text-sm font-bold text-emerald-900">Pico no TikTok</h4>
                    <p className="text-xs text-emerald-400 mt-1">O vídeo "Notícias de Hoje" está viralizando. +4.2k seguidores nas últimas 2 horas.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-indigo-600 rounded-xl shadow-sm p-6 text-white relative overflow-hidden">
            ...
            <button className="bg-zinc-900 text-indigo-600 px-4 py-2 rounded-lg text-sm font-bold shadow-sm hover:bg-indigo-900/30 transition-colors relative z-10">
