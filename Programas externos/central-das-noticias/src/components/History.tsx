    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Acesse todos os roteiros gerados anteriormente.</p>
        </div>
        
        {scripts.length > 0 && (
          <button 
            onClick={clearHistory}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-600 bg-red-900/30 hover:bg-red-100 rounded-lg transition-colors"
          >
            <Trash2 size={16} />
            Limpar Histórico
          </button>
      ...
      {scripts.length === 0 ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-12 text-center flex flex-col items-center justify-center">
          ...
          <h3 className="text-lg font-medium text-zinc-100 mb-1">Nenhum roteiro salvo</h3>
          <p className="text-zinc-400">Os roteiros que você gerar aparecerão aqui automaticamente.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {scripts.map((script) => (
            <div key={script.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
              <div 
                className="p-4 flex items-center justify-between cursor-pointer hover:bg-transparent transition-colors"
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-zinc-100">{script.title || 'Roteiro sem título'}</h3>
              ...
              {expandedId === script.id && (
                <div className="border-t border-zinc-100 p-4 bg-transparent">
                  <div className="flex justify-end gap-2 mb-4">
                    <button 
                      onClick={() => handlePlay(script.id, script.content)}
                      className={`flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${isPlaying === script.id ? 'bg-indigo-100 text-indigo-400 border border-indigo-200' : 'bg-zinc-900 text-zinc-700 border border-zinc-800 hover:bg-transparent'}`}
                    >
                      {isPlaying === script.id ? <><Square size={14} /> Parar Áudio</> : <><Play size={14} /> Ouvir Roteiro</>}
                    </button>
                    <button 
                      onClick={() => handleCopy(script.content)}
                      className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-zinc-700 bg-zinc-900 border border-zinc-800 hover:bg-transparent rounded-md transition-colors"
                    >
                      <Copy size={14} /> Copiar
                    </button>
                    <button 
                      onClick={() => handleDelete(script.id)}
                      className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-red-600 bg-zinc-900 border border-red-200 hover:bg-red-900/30 rounded-md transition-colors"
                    >
                      <Trash2 size={14} /> Excluir
                    </button>
                  </div>
                  <div className="prose prose-sm max-w-none bg-zinc-900 p-4 rounded-lg border border-zinc-800">
