...
export function ImageStudio() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-md bg-zinc-900 border border-zinc-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 min-h-[100px] p-3"
            />
          </div>
              ...
              {(['16:9', '1:1', '9:16'] as const).map((ratio) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors border ${
                    aspectRatio === ratio
                      ? 'bg-indigo-900/30 border-indigo-200 text-indigo-400'
                      : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:bg-transparent'
                  }`}
                >
      ...
      {error && (
        <div className="bg-red-900/30 text-red-600 p-4 rounded-xl border border-red-100">
          {error}
        </div>
      )}

      {isGenerating && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-12 flex flex-col items-center justify-center text-zinc-400">
      ...
      {generatedImage && !isGenerating && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-lg text-zinc-100">Resultado</h3>
            <div className="flex gap-2">
              <button
                onClick={handleGenerate}
                className="p-2 text-zinc-400 hover:bg-zinc-800 rounded-md transition-colors"
          ...
          <div className={`bg-zinc-800 rounded-lg overflow-hidden flex items-center justify-center ${
