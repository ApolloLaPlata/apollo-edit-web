  ...
  return (
    <div className="space-y-6">
      <div className="bg-zinc-900 rounded-xl shadow-sm border border-zinc-800 p-6">
        ...
        {configs.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed border-zinc-800 rounded-xl bg-transparent">
            <Webhook size={48} className="mx-auto text-zinc-300 mb-3" />
            <h3 className="text-lg font-medium text-zinc-100 mb-1">Nenhuma integração configurada</h3>
            <p className="text-zinc-400 mb-4">Adicione um webhook para enviar dados automaticamente para seus outros sistemas.</p>
            <button
              onClick={addWebhook}
              className="inline-flex items-center gap-2 bg-zinc-900 border border-zinc-700 hover:bg-transparent text-zinc-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors"
            >
              <Plus size={16} />
              Adicionar Webhook
            </button>
          ...
          <div className="space-y-4">
            {configs.map(config => (
              <div key={config.id} className="border border-zinc-800 rounded-xl overflow-hidden">
                <div className="bg-transparent p-4 flex items-center justify-between border-b border-zinc-800">
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      className="font-medium text-zinc-100 bg-transparent border-none focus:ring-0 p-0"
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 text-xs font-medium text-emerald-600 hover:text-emerald-400 bg-emerald-900/30 hover:bg-emerald-100 px-2 py-1 rounded transition-colors"
                    >
                      <Play size={14} /> Testar
                    </button>
                    <button
                ...
                <div className="p-4 space-y-4 bg-zinc-900">
                  <div>
                    <label className="block text-xs font-medium text-zinc-400 mb-1">URL do Webhook (POST)</label>
                    <input
                      type="text"
                      value={config.url}
                      onChange={(e) => updateWebhook(config.id, 'url', e.target.value)}
                      className="w-full px-3 py-2 bg-transparent border border-zinc-800 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                      placeholder="http://localhost:5000/webhook"
                    />
                  </div>
                  
                  <div>
                    <label className="block text-xs font-medium text-zinc-400 mb-2">Eventos que disparam este webhook</label>
                    <div className="flex flex-wrap gap-2">
                      <label className="flex items-center gap-2 text-sm bg-transparent px-3 py-1.5 rounded-lg border border-zinc-800 cursor-pointer hover:bg-zinc-800">
                        <input
                          type="checkbox"
