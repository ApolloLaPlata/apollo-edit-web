    ...
    <div className="space-y-6">
      <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-zinc-100">Painel de Análise</h2>
              <p className="text-sm text-zinc-400">Extraia e cruze dados reais da sua página.</p>
            </div>
          </div>
          {baseData && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-zinc-700 bg-zinc-800 hover:bg-zinc-200 rounded-lg transition-colors disabled:opacity-50"
            >
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            <input
              ...
              className="w-full pl-4 pr-4 py-3 bg-transparent border border-zinc-800 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-zinc-100 placeholder-zinc-400"
              disabled={isAnalyzing}
            />
          </div>
          {!baseData && (
        ...
        {error && (
          <div className="mt-4 p-4 bg-red-900/30 text-red-400 rounded-lg flex items-start gap-3 border border-red-100">
      ...
      {baseData && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 shadow-sm flex items-start gap-4">
              <div className="bg-blue-50 p-3 rounded-lg text-blue-600 mt-1">
                <LinkIcon size={20} />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="font-bold text-zinc-100">Fonte: Web Scraping</h3>
                  <span className="px-2 py-1 bg-emerald-100 text-emerald-400 text-xs font-bold rounded-full">Ativo</span>
                </div>
                <p className="text-sm text-zinc-400 mb-2">Dados extraídos diretamente da URL pública fornecida.</p>
                <div className="text-xs text-zinc-400 bg-transparent p-2 rounded border border-zinc-100">
                  URL: {url}
                </div>
              </div>
            </div>

            <div className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 shadow-sm flex items-start gap-4 opacity-75">
              <div className="bg-purple-50 p-3 rounded-lg text-purple-600 mt-1">
                <Database size={20} />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="font-bold text-zinc-100">Fonte: API Oficial</h3>
                  <span className="px-2 py-1 bg-zinc-800 text-zinc-400 text-xs font-bold rounded-full">Aguardando Configuração</span>
                </div>
                <p className="text-sm text-zinc-400 mb-2">Conecte as APIs (YouTube, TikTok) nas Configurações para cruzar dados oficiais com a leitura da página.</p>
                <button className="text-xs font-medium text-emerald-600 hover:text-emerald-400">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 shadow-sm flex items-center gap-4 hover:border-emerald-200 transition-colors cursor-default">
              <div className="bg-indigo-900/30 p-3 rounded-lg text-indigo-600">
                <Users size={24} />
              </div>
              <div>
                <p className="text-sm font-medium text-zinc-400">Seguidores</p>
                <p className="text-2xl font-bold text-zinc-100">{baseData.followers}</p>
              </div>
            </div>
            ...
            <div className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 shadow-sm flex items-center gap-4 hover:border-emerald-200 transition-colors cursor-default">
              ...
              <div>
                <p className="text-sm font-medium text-zinc-400">Curtidas Totais</p>
                <p className="text-2xl font-bold text-zinc-100">{baseData.likes}</p>
              </div>
            </div>

            <div className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 shadow-sm flex items-center gap-4 hover:border-emerald-200 transition-colors cursor-default">
              <div className="bg-orange-900/30 p-3 rounded-lg text-orange-600">
                <Video size={24} />
              </div>
              <div>
                <p className="text-sm font-medium text-zinc-400">Vídeos Publicados</p>
                <p className="text-2xl font-bold text-zinc-100">{baseData.videos}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-zinc-100">Vídeos Recentes (Extraídos da Página)</h3>
            ...
            {baseData.recentVideos && baseData.recentVideos.length > 0 ? (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {displayedVideos.map((video, index) => (
                    <div key={index} className="border border-zinc-100 rounded-lg p-4 hover:border-emerald-200 transition-colors bg-transparent">
                      <p className="font-medium text-zinc-100 mb-3 line-clamp-2" title={video.title}>
                ...
                {hasMoreVideos && (
                  <button 
                    ...
                    className="mt-4 w-full py-3 border border-zinc-800 rounded-lg text-sm font-medium text-zinc-700 hover:bg-transparent hover:text-emerald-600 hover:border-emerald-200 transition-all flex items-center justify-center gap-2"
                  >
                    <Maximize2 size={16} />
              ...
              <div className="text-center py-8 text-zinc-400 bg-transparent rounded-lg border border-zinc-100 border-dashed">
      ...
      {showAllVideos && baseData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-zinc-900 rounded-2xl shadow-xl w-full max-w-4xl max-h-[85vh] flex flex-col animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-zinc-100">
              <div>
                <h2 className="text-xl font-bold text-zinc-100">Todos os Vídeos Extraídos</h2>
                <p className="text-sm text-zinc-400">Mostrando {baseData.recentVideos.length} vídeos de {baseData.username}</p>
              </div>
              <button 
                onClick={() => setShowAllVideos(false)}
                className="p-2 text-zinc-400 hover:text-zinc-400 hover:bg-zinc-800 rounded-full transition-colors"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {baseData.recentVideos.map((video, index) => (
                  <div key={index} className="border border-zinc-800 rounded-lg p-4 hover:border-emerald-300 transition-colors bg-zinc-900 shadow-sm">
                    <div className="flex gap-4">
                      <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-400 font-bold text-sm shrink-0">
                        {index + 1}
                      </div>
                      <div>
                        <p className="font-medium text-zinc-100 mb-3" title={video.title}>
                          {video.title || 'Sem título'}
                        </p>
                        <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-400">
                          {video.views && video.views !== 'N/A' && (
                            <div className="flex items-center gap-1 bg-transparent px-2 py-1 rounded">
