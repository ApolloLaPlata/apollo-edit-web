...
export const generateImage = async (
    ...
    if (settings.imageProvider === 'comfyui' && settings.comfyUrl && settings.comfyImageWorkflow) {
        ...
        // Envia para o nosso AI Director rodando na porta 8000
        const imagesB64 = characters.map(c => c.images[0]);
        ...
        if (!response.ok) {
            throw new Error(`Failed to generate via Apollo Cloud: ${response.statusText}`);
        }
