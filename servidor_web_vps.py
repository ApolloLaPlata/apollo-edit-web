...
async def voice_generate(request: Request):
    ...
    valid_modal = [a for a in accounts if a.get('provider') == 'modal' and a.get('is_active', True)]
    MODAL_USER = valid_modal[0].get('workspace') if valid_modal else "apollolaplata"
                    ...
                    for chunk in chunks:
                        ...
                        if ref_bytes:
                            try:
                                ...
                                valid_modal = [a for a in accounts if a.get('provider') == 'modal' and a.get('is_active', True)]
                                MODAL_USER = valid_modal[0].get('workspace') if valid_modal else "apollolaplata"
                                xtts_url = f"https://{MODAL_USER}--apollo-api-xtts.modal.run"
