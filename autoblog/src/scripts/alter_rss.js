...
try {
  ...
  await db.prepare(`ALTER TABLE AgentConfig ADD COLUMN rssFeeds TEXT`).run();
  ...
  const blog = await db.prepare('SELECT id FROM Blog WHERE domain = ?').get('filosofiadocodigo.com');
  if (blog) {
    await db.prepare('UPDATE AgentConfig SET rssFeeds = ? WHERE blogId = ?').run('https://g1.globo.com/rss/g1/tecnologia/', blog.id);
