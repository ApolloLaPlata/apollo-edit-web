...
try {
  db.prepare("ALTER TABLE Blog ADD COLUMN socialLinks TEXT DEFAULT '{}'").run();
...
const blogs = db.prepare('SELECT id, name, domain FROM Blog').all();
for (const b of blogs) {
  db.prepare('UPDATE Blog SET socialLinks = ? WHERE id = ?').run(JSON.stringify(defaultSocials), b.id);
