    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-600 mt-1">Crie roteiros personalizados com a identidade do seu canal.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-6">
            <h3 className="text-lg font-semibold text-zinc-900 mb-4">Gerador de Roteiro</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                {profiles.length > 0 ? (
                  <select
                    ...
                    className="w-full px-3 py-2 bg-white border border-zinc-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-white border border-zinc-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none min-h-[120px] resize-y"
                />
              </div>
          ...
          {generatedScript && (
            <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-zinc-900">Roteiro Gerado</h3>
                <div className="flex items-center gap-2">
                  <button
                    onClick={handleDownloadTXT}
                    className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-zinc-700 bg-zinc-100 hover:bg-zinc-200 rounded-lg transition-colors"
                  >
                    <Download size={16} />
                    Baixar TXT
                  </button>
              ...
              <div className="prose prose-zinc max-w-none bg-zinc-50 p-6 rounded-lg border border-zinc-200">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-zinc-900">Perfis de Canal</h3>
              {!isCreatingProfile && (
                <button
            ...
            {isCreatingProfile ? (
              <div className="space-y-4 bg-zinc-50 p-4 rounded-lg border border-zinc-200">
                <h4 className="font-medium text-zinc-900 text-sm">Novo Perfil</h4>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-sm bg-white border border-zinc-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-1.5 text-sm bg-white border border-zinc-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none resize-none h-16"
                  />
                </div>

                <div>
                  <label className="block text-xs font-medium text-zinc-700 mb-1">Arquivos de Treinamento (TXT, MD)</label>
                  <p className="text-xs text-zinc-500 mb-2">Faça upload de roteiros antigos, guias de estilo ou textos que representem a identidade do canal.</p>
                  ...
                  <button
                    ...
                    className="w-full py-2 border-2 border-dashed border-zinc-300 rounded-lg text-sm text-zinc-600 hover:border-indigo-500 hover:text-indigo-600 transition-colors flex items-center justify-center gap-2"
                  >
                    <UploadCloud size={16} />
                    Selecionar Arquivos
                  </button>
                  ...
                  {newProfileFiles.length > 0 && (
                    <ul className="mt-3 space-y-2">
                      {newProfileFiles.map((f, i) => (
                        <li key={i} className="flex items-center justify-between text-xs bg-white p-2 border border-zinc-200 rounded">
                          <span className="truncate max-w-[180px]" title={f.name}>{f.name}</span>
                          <button onClick={() => removeNewFile(i)} className="text-red-500 hover:text-red-700">
                            <Trash2 size={14} />
                          </button>
                        </li>
                      ))}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 bg-zinc-200 hover:bg-zinc-300 text-zinc-800 text-sm font-medium py-2 rounded-md transition-colors"
                  >
                    Cancelar
                  </button>
                </div>
              ...
              <div className="space-y-3">
                {profiles.length === 0 ? (
                  <p className="text-sm text-zinc-500 text-center py-4">Nenhum perfil cadastrado.</p>
                ) : (
                  profiles.map(p => (
                    <div key={p.id} className={`p-3 rounded-lg border transition-colors ${activeProfileId === p.id ? 'border-indigo-500 bg-indigo-50' : 'border-zinc-200 bg-zinc-50 hover:border-zinc-300'}`}>
                      <div className="flex items-start justify-between">
                        <div 
                          className="cursor-pointer flex-1"
                          onClick={() => setActiveProfileId(p.id)}
                        >
                          <h4 className="font-medium text-zinc-900 text-sm">{p.name}</h4>
