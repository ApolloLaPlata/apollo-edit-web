...
app = modal.App("stable-audio-3-engine-final")
...
class StableAudio3EngineFinal:
    ...
    def generate(self, prompt: str, seconds: int = 120):
        ...
        # PARÂMETROS REVELADOS PELO PERPLEXITY PARA O MODELO DISTILADO:
        steps = 8
        cfg = 1.0
        ...
        with torch.no_grad():
            output = generate_diffusion_cond_inpaint(
                ...
                sampler_type="dpmpp-3m-sde", # usando o default robusto p/ inpaint
                device="cuda"
            )
            
        # PÓS-PROCESSAMENTO EXATO DA STABILITY AI (Perplexity Docs)
...
def run_tests():
    ...
    engine = StableAudio3EngineFinal()
