...
#title="Apollo Studio Web Engine")
...
# Vari├âãÆ├é┬ível Global de Workspace (injetada pelo apollo_studio.py)
...
async def read_root(request: Request):
    ...
    if os.path.exists(hub_path):
        ...
        return response
    return HTMLResponse(content="<h1>Hub n├úo encontrado</h1>", status_code=404)
...
async def read_apollo_master():
    ...
    return HTMLResponse(content="<h1>Admin n├â┬úo encontrado</h1>", status_code=404)
...
async def read_apollo_master_login():
    ...
    if os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as f:
            return HTMLResponse(content=f.read(), status_code=200)
    return HTMLResponse(content="<h1>Login n├âãÆ├é┬úo encontrado</h1>", status_code=404)
...
async def read_mobile(request: Request):
    ...
    if os.path.exists(path):
        ...
        return response
    return HTMLResponse(content="<h1>Mobile Studio n├â┬úo encontrado</h1>", status_code=404)

@app.get("/{filename}.html")
async def serve_html(request: Request, filename: str):
    """Serve arquivos HTML com tradu├âãÆ├é┬º├âãÆ├é┬úo nativa baseada em cookie"""
    lang = request.cookies.get("apollo_lang", "pt")
...
async def generate_presigned_url(req: PresignedURLRequest):
    ...
    fa├âãÆ├é┬ºa o upload de arquivos pesados (ex: 2GB) direto do navegador,
    sem sobrecarregar o servidor VPS com tr├âãÆ├é┬ífego.
    """
    # ATEN├âãÆ├éÔÇí├âãÆ├éãÆO: As credenciais devem vir do config.json do Workspace no futuro
    # Por seguran├âãÆ├é┬ºa, estamos mockando a configura├âãÆ├é┬º├âãÆ├é┬úo at├âãÆ├é┬® o usu├âãÆ├é┬írio prover as chaves R2/S3
    ...
    try:
        response = s3_client.generate_presigned_url(
            ...
            ExpiresIn=3600 # V├âãÆ├é┬ílido por 1 hora
        )
...
# =====================================================================
# ### IN├â┬ìCIO DA L├âÔÇ£GICA DE ADMINISTRA├âÔÇí├âãÆO E AUTENTICA├âÔÇí├âãÆO (SAAS) ###
# =====================================================================
# AVISO DE REFATORA├âÔÇí├âãÆO FUTURA: Estas rotas controlam o painel Master,
# chaves de API e saldo de usu├â┬írios. Futuramente mover para: /src/api/admin_routes.py
# =====================================================================
...
async def admin_get_dashboard(token: str = None):
    try:
        ...
        prompt = f"""Voc├â┬¬ ├â┬® o Agente Financeiro Aut├â┬┤nomo.
Resumo atual do sistema:
- Usu├â┬írios Totais: {eco_stats.get("total_users")}
- Receita Estimada (Coins): ${estimated_revenue:.2f}
- Custo de APIs de IA: ${total_api_spend:.2f}
- Custo de Computa├â┬º├â┬úo Cloud: ${total_cloud_spend:.2f}
- Lucro Estimado: ${profit:.2f}

Escreva um relat├â┬│rio de 1 par├â┬ígrafo focado na sa├â┬║de financeira. Seja direto e anal├â┬¡tico."""
...
async def market_analyst_recommendations(token: str = None):
    try:
        ...
        recommendations.append({
            "id": "high_dalle_cost",
            "type": "warning",
            "title": "Custo Elevado: Gera├â┬º├â┬úo Visual",
            "description": "Custo das APIs de imagem est├â┬í acima da m├â┬®dia. Recomenda├â┬º├â┬úo do Agente: Alterar fallback para infraestrutura Modal local (Flux). Economia estimada: R$ 450/dia.",
        ...
        recommendations.append({
            ...
            "description": f"Reten├â┬º├â┬úo no minigame subiu. Usu├â┬írios totais: {eco_stats.get('total_users', 0)}. Recomenda├â┬º├â┬úo: Aumentar o custo em 'Gasolina' dos renders para for├â┬ºar mais tempo de gameplay (AdSense).",
...
async def market_analyst_apply(payload: MarketActionPayload):
    try:
        ...
        return {"success": False, "error": "A├â┬º├â┬úo desconhecida."}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def admin_manage_credits(req: UserCreditRequest):
    try:
        ...
        if res:
            return {"success": True}
        return {"success": False, "error": "Falha ao gerenciar cr├âãÆ├é┬®ditos"}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def admin_create_user(req: NewUserRequest):
    try:
        ...
        if res:
            return {"success": True}
        return {"success": False, "error": "Usu├âãÆ├é┬írio j├âãÆ├é┬í existe ou erro no banco"}
    except Exception as e:
        return {"success": False, "error": str(e)}
        
# --- IN├â┬ìCIO FASE VIII: ORQUESTRA├âÔÇí├âãÆO N├â┬ìVEL DEUS (MOBILE / POCKET DIRECTOR) ---
...
async def autoblog_submit(req: AutoblogSubmitRequest):
    try:
        ...
        # Notifica via WebSocket/JobNotifier (se j├â┬í conectado) ou deixa pendente
...
def VerifyBiometricSignature(signature: str, payload_str: str) -> bool:
    # Em produ├â┬º├â┬úo real, validaria assinatura RSA/ECDSA do dispositivo
...
async def mobile_approve(req: MobileApproveRequest):
    try:
        ...
        # Validar assinatura biom├â┬®trica
        ...
        if req.cost_gas > 0:
            if not user_database.deduct_currency(req.user_id, req.cost_gas, 'gas', f"Aprova├â┬º├â┬úo Mobile ID {req.approval_id}"):
                raise HTTPException(status_code=402, detail="Insufficient Gas")
                
        if req.cost_crystals > 0:
            if not user_database.deduct_currency(req.user_id, req.cost_crystals, 'crystals', f"Aprova├â┬º├â┬úo Mobile ID {req.approval_id}"):
        ...
        # TODO: Encaminhar para fila real de renderiza├â┬º├â┬úo no PC
...
async def mobile_render_start(req: MobileRenderStartRequest, background_tasks: BackgroundTasks):
    try:
        ...
        return {"success": True, "message": "Renderiza├â┬º├â┬úo iniciada em background na m├â┬íquina local."}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
# --- FIM ROTAS DE ADMINISTRA├âÔÇí├âãÆO ---
        
# --- FIM ROTAS DE ADMINISTRA├âãÆ├óÔé¼┬í├âãÆ├åÔÇÖO ---

# =====================================================================
# ### IN├â┬ìCIO DA L├âÔÇ£GICA DO PAINEL DO USU├â┬üRIO E GAMIFICA├âÔÇí├âãÆO ###
# =====================================================================
# AVISO DE REFATORA├âÔÇí├âãÆO FUTURA: Perfil, experi├â┬¬ncia, invent├â┬írio e itens RPG.
...
async def get_user_profile():
    ...
    # Busca os dados base do usu├â┬írio (assumindo ID 1 como master provis├â┬│rio)
    cursor.execute("""
    ...
    # Se o usu├â┬írio n├â┬úo existir (banco rec├â┬®m-criado), criamos o usu├â┬írio admin
    if not user:
    ...
    # Busca os cosm├â┬®ticos desbloqueados no invent├â┬írio
    cursor.execute("""
...
async def log_page_visit_endpoint(req: LogVisitRequest):
    ...
    # Retornar settings de p├â┬ígina para que o auth.js possa aplicar bloqueios de acesso
    try:
        conn = user_database.get_connection()
        cursor = conn.cursor()
...
async def buy_item(req: Request):
    ...
    try:
        ...
        # Simular desconto de cristais (Como n├âãÆ├é┬úo temos coluna cristais, vamos usar dummy success)
...
def check_feature(feature_key: str):
    ...
    if res and res[0] == 'off':
        raise HTTPException(status_code=503, detail="Ferramenta em manuten├â┬º├â┬úo pelo Administrador.")
    return True

def check_user_status(user_id: int = 1):
    """Verifica se o usu├â┬írio est├â┬í banido."""
    import user_database
    conn = user_database.get_connection()
    cursor = conn.cursor()
...
async def chat_interaction(msg: ChatMessage):
    ...
    import time
    time.sleep(1) # Simular lat├âãÆ├é┬¬ncia
    return {
        "response": f"Processando sua requisi├âãÆ├é┬º├âãÆ├é┬úo: '{msg.message}'. Como Apollo Copilot da V2, estou configurando os agentes para iniciar a constru├âãÆ├é┬º├âãÆ├é┬úo do seu v├âãÆ├é┬¡deo utilizando os bancos de dados do seu canal..."
    }

# =====================================================================
# ### IN├â┬ìCIO DA L├âÔÇ£GICA DE PROXY (MOTORES EXTERNOS / LIGHTNING AI) ###
# =====================================================================
# AVISO DE REFATORA├âÔÇí├âãÆO FUTURA: Estes endpoints bypassam o CORS para falar
# com o Lightning AI e geradores de imagem/├â┬íudio externos.
...
async def media_proxy(request: Request):
    """
    Proxy Universal para o "Pendrive M├â┬ígico da Nuvem".
    Recebe do frontend a URL alvo, os cabe├â┬ºalhos e o corpo da requisi├â┬º├â┬úo.
    Bypassa o CORS para gerar Imagem, ├â┬üudio, V├â┬¡deo e M├â┬║sica em qualquer API.
    """
    try:
        import requests
        data = await request.json()
        ...
        if not target_url:
            return JSONResponse(status_code=400, content={"error": "target_url n├â┬úo fornecida"})
            
        target_headers = data.pop("target_headers", {})
        
        # Faz a requisi├â┬º├â┬úo para a API externa
        response = requests.post(
            target_url,
            json=data,
            headers=target_headers,
            timeout=120 # Timeout maior para gera├â┬º├â┬úo de m├â┬¡dia pesada
        )
...
# =====================================================================
# ### IN├â┬ìCIO DA L├âÔÇ£GICA DO BOT DE WHATSAPP E CHATBOT (LLM CASCADE) ###
# =====================================================================
# AVISO DE REFATORA├âÔÇí├âãÆO FUTURA: Controle do C├â┬úo de Guarda (Node.js) e do
...
async def start_whatsapp():
    ...
    # Mas como o bat j├â┬í existe, executamos ele minimizado:
...
async def chat_send(request: Request):
    try:
        ...
        if agent_id == "PRIME":
            ...
            if admin_group_id:
                try:
                    ...
                    out_data = json.dumps({"to": admin_group_id, "message": f"├░┼©┬ñÔÇô [Painel Web]: {msg_text}"}).encode('utf-8')
                    urllib.request.urlopen(req, data=out_data)
                except Exception as e:
                    print(f"[WhatsApp] Falha ao enviar c├â┬│pia da pergunta web: {e}")
        ...
        # O hist├â┬│rico precisa ser o unificado se for o PRIME
        ...
        if agent_id == "PRIME" and len(unified_chat_history) > 1:
            ...
            prompt_with_history = f"Hist├â┬│rico Recente:\n{history_text}\n\nMensagem Atual:\n{msg_text}"
            
        # O painel Web ├â┬® usado apenas pelo admin (Chefe). Conectando direto no Lightning AI.
        import requests
        chat_cfg = cm.get("api_config", {}).get("lightning_chat", {})
        api_keys = chat_cfg.get("api_keys", [])
        if api_key:
        ...
        response_text = ""
        if not used_api_key:
            response_text = "├ó┼í┬á├»┬©┬Å [Apollo Prime] A Chave da API da Lightning n├â┬úo est├â┬í configurada neste Canal."
        else:
            headers = {
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    response_text = f"├ó┼í┬á├»┬©┬Å [Lightning API Error] Falha de comunica├â┬º├â┬úo (Status {resp.status_code})"
            except Exception as e:
                response_text = f"├ó┼í┬á├»┬©┬Å [Network Error] Falha ao contatar a Lightning AI: {str(e)}"
            ...
            if agent_id == "PRIME":
                ...
                if admin_group_id:
                    try:
                        ...
                        print(f"[WhatsApp] Falha ao enviar c├â┬│pia web: {e}")
...
async def whatsapp_webhook(request: Request):
    ...
    try:
        ...
        # Se a mensagem foi digitada pelo dono do celular, o ID do Grupo est├â┬í no 'to'. 
        # Se foi digitada por outra pessoa no grupo, est├â┬í no 'from'.
        group_id = recipient if "@g.us" in recipient else sender
        
        whatsapp_last_contact = group_id
        user_text = msg.get("body", "")
        
        # Procura qual workspace ├â┬® o dono desse group_id
        ...
        # 4. Se n├â┬úo encontrou NENHUM workspace dono deste grupo, ignora a mensagem silenciosamente
        if not target_workspace_path:
            print(f"[Aviso] Mensagem ignorada de grupo ou contato desconhecido/n├â┬úo configurado: {group_id}")
        ...
        system_prompt = cm.get("ai_system_prompt", "Voc├â┬¬ ├â┬® o assistente virtual deste canal.")
        system_prompt += "\n\nATEN├âÔÇí├âãÆO: Voc├â┬¬ est├â┬í falando com o usu├â┬írio atrav├â┬®s de um grupo do WhatsApp. Responda de forma direta, ir├â┬┤nica e concisa, como o rob├â┬┤ do sistema. Evite formata├â┬º├â┬Áes longas."
        
        # Carrega hist├â┬│rico persistente do canal (usaremos uma vers├â┬úo simplificada aqui)
        prompt_with_history = f"Mensagem Atual do Usu├â┬írio:\n{user_text}"
        if target_workspace_path == BASE_DIR:
            history_text = "\n".join([f"{item['role'].upper()}: {item['content']}" for item in unified_chat_history[-10:]])
            prompt_with_history = f"Hist├â┬│rico Recente:\n{history_text}\n\n{prompt_with_history}"
        ...
        response_text = ""
        if not api_key:
            response_text = "├ó┼í┬á├»┬©┬Å [Apollo Prime] A Chave da API da Lightning n├â┬úo est├â┬í configurada neste Canal. Configure no Painel Web primeiro."
        else:
            headers = {
        ...
        # O hist├â┬│rico unificado j├â┬í fez isso acima, ent├â┬úo o painel ser├â┬í atualizado via sync
...
async def gerar_grupo_whatsapp(request: Request):
    ...
    try:
        ...
        req.add_header('Content-Type', 'application/json')
        out_data = json.dumps({"name": "├░┼©┬ñÔÇô ADM APOLLO EDIT WEB"}).encode('utf-8')
...
async def get_workspace_info():
    """Retorna dados b├âãÆ├é┬ísicos do workspace logado."""
    return {
...
# =====================================================================
# ### IN├â┬ìCIO DA L├âÔÇ£GICA DE RENDER E GERA├âÔÇí├âãÆO DE CONTE├â┼íDO (A F├â┬üBRICA) ###
# =====================================================================
# AVISO DE REFATORA├âÔÇí├âãÆO FUTURA: Estas s├â┬úo as rotas mais pesadas. Elas 
# invocam scripts Python (FFmpeg, TTS, RVC) e n├â┬úo devem ser interrompidas
# por IAs. O motor ├â┬® puramente determin├â┬¡stico aqui.
...
async def api_ajustador_processar(
    ...
    def worker():
        try:
            ...
            q.put(f"├â┬ó├à┬í├óÔÇ×┬ó├â┬»├é┬©  Configurando processador de m├âãÆ├é┬¡dia...\n")
            q.put(f"├â┬░├à┬©├óÔé¼┼ô├óÔé¼┼í Sa├âãÆ├é┬¡da autom├âãÆ├é┬ítica configurada para: {out_dir}\n")
            ...
            if bg_path:
                ...
                q.put(f"├â┬░├à┬©├à┬¢├à┬¥├â┬»├é┬©  V├âãÆ├é┬¡deo de fundo detectado e configurado.\n")
            ...
            for i, p in enumerate(input_paths, 1):
                ...
                q.put(f"\n├â┬░├à┬©├óÔé¼┼ô├à┬¢ [{i}/{total}] Processando cena {i}...")
            ...
            if erros == 0:
                q.put(f"\n\n├â┬ó├àÔÇ£├óÔé¼┬ª Conclu├âãÆ├é┬¡do com sucesso! {total} v├âãÆ├é┬¡deos salvos na pasta 'saida_ajustador' do seu projeto.")
            else:
                q.put(f"\n\n├â┬ó├à┬í├é┬á├â┬»├é┬©  Finalizado com {erros} erros. Verifique os logs acima.")
                
        except Exception as e:
            q.put(f"\n├â┬ó ├àÔÇÖ Erro fatal: {str(e)}")
        finally:
            q.put(None)
...
# ===== ROTAS DA F├âãÆ BRICA DE M├âãÆ├à┬íSICAS =====
...
async def fabrica_carregar_musicas(req: CarregarMusicasReq):
    if not os.path.exists(req.suno_dir):
        return {"success": False, "error": "Diret├âãÆ├é┬│rio n├âãÆ├é┬úo encontrado."}
        ...
        try:
            ...
            for m_data in req.musicas:
                ...
                try:
                    ...
                    if chosen_template == "Aleat├âãÆ├é┬│rio (Auto)":
                    ...
                    q.put(json.dumps({"type": "status", "id": item_id, "status": "Conclu├âãÆ├é┬¡do"}))
                    q.put(json.dumps({"type": "log", "text": f"├â┬ó├é┼ô├éÔÇª Sucesso: {song_name}"}))
                except Exception as e:
                    q.put(json.dumps({"type": "status", "id": item_id, "status": "Erro"}))
                    q.put(json.dumps({"type": "log", "text": f"├â┬ó├é┬Ø├é┼Æ Erro em {m_data['nome']}: {e}"}))
                    
        except Exception as ex:
            q.put(json.dumps({"type": "log", "text": f"├â┬ó├é┬Ø├é┼Æ Erro fatal: {ex}"}))
        finally:
            q.put(None)
    ...
    def worker():
        try:
            if not os.path.exists(req.root_dir):
                q.put("├â┬ó├é┬Ø├é┼Æ Diret├âãÆ├é┬│rio n├âãÆ├é┬úo existe.\n")
                return
            ...
            if not videos:
                q.put("├â┬ó├é┬Ø├é┼Æ Nenhum .mp4 encontrado nas subpastas.\n")
                return
            ...
            q.put("Montando grade de compila├âãÆ├é┬º├âãÆ├é┬úo...\n")
            ...
            q.put(f"\n├â┬ó├é┼ô├éÔÇª Compila├âãÆ├é┬º├âãÆ├é┬úo de {req.duracao_min} minutos conclu├âãÆ├é┬¡da!\nSalvo em: {out_path}\n")
        except Exception as e:
            q.put(f"\n├â┬ó├é┬Ø├é┼Æ Erro na compila├âãÆ├é┬º├âãÆ├é┬úo: {e}\n")
        finally:
            q.put(None)
...
async def browse_file_api(type: str = "file"):
    """Abre um File Dialog nativo no Python para o usu├âãÆ├é┬írio escolher arquivo"""
...
async def get_thumb(path: str):
    """Retorna o arquivo diretamente se for imagem, ou extrai um frame se for v├âãÆ├é┬¡deo"""
    if not os.path.exists(path):
    ...
    if ext in ['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp']:
        ...
        # Se for v├âãÆ├é┬¡deo, idealmente usar FFmpeg para extrair frame.
        # Aqui enviamos o pr├âãÆ├é┬│prio v├âãÆ├é┬¡deo se o navegador suportar, ou um ├âãÆ├é┬¡cone padr├âãÆ├é┬úo.
        # Para simplificar na prova de conceito, enviaremos um placeholder se for v├âãÆ├é┬¡deo.
...
async def delete_profile(name: str):
    try:
        ...
        # Tamb├âãÆ├é┬®m deleta a imagem de preview se existir
...
# ===== ROTAS PARA O TTS (GERADOR DE ├âãÆ├é┬üUDIO) =====
...
def tts_gerar(req: TTSRequest):
    """Gera ├âãÆ├é┬íudio usando a API VoiceMaker ou a configurada"""
    try:
        from config_manager import ConfigManager
        ...
        # Garante n├âãÆ├é┬úo sobrescrever
        contador = 1
        ...
        if success:
            ...
            return {"success": False, "error": "Falha na gera├âãÆ├é┬º├âãÆ├é┬úo via API TTSManager"}
...
def tts_testar_google(req: TTSRequest):
    ...
    try:
        ...
        if not voice_id:
            return {"success": False, "error": "Personagem n├âãÆ├é┬úo possui 'Voz Google TTS' configurada."}
        ...
        if success:
            ...
            return {"success": False, "error": "Falha ao requisitar ├âãÆ├é┬íudio do Gemini TTS"}
...
async def voice_generate(request: Request):
    ...
    if not text:
        ...
        return JSONResponse({"error": "Texto n├úo fornecido"}, status_code=400)
    ...
    if voice_id.startswith("kokoro_"):
        ...
        xtts_dir = os.path.join(BASE_DIR, "backend", "voices", "xtts") # Mantemos a pasta antiga como reposit├│rio
        wav_path = os.path.join(xtts_dir, f"{wav_name}.wav")
        if not os.path.exists(wav_path):
            from fastapi.responses import JSONResponse
            return JSONResponse({"error": f"├üudio de refer├¬ncia n├úo encontrado: {wav_path}"}, status_code=404)
...
async def studio_voice_generate(text: str = Form(...), voice_id: str = Form(None), voice_file: UploadFile = File(None)):
    ...
    if voice_file:
        ...
        if os.path.exists(default_path):
            ...
            from fastapi.responses import JSONResponse
            return JSONResponse({"error": "Voz padr├úo n├úo encontrada e nenhum arquivo foi enviado."}, status_code=400)
...
# ===== ROTAS PARA O NARRADOR (GERADOR DE V├âãÆ├é┬ìDEO) =====
...
async def narrador_gerar(
    ...
    try:
        ...
        if personagem not in personagens_cache:
            return {"success": False, "error": f"Personagem n├âãÆ├é┬úo encontrado: {personagem}"}
            
        personagem_config = personagens_cache[personagem]
        
        if "estados_emocionais" not in personagem_config:
            return {"success": False, "error": f"Estados emocionais n├âãÆ├é┬úo configurados para: {personagem}"}
            
        if estado_emocional not in personagem_config["estados_emocionais"]:
            return {"success": False, "error": f"Estado emocional n├âãÆ├é┬úo encontrado: {estado_emocional}"}
        ...
        if not video_source or not os.path.exists(video_source):
            return {"success": False, "error": f"V├âãÆ├é┬¡deo base n├âãÆ├é┬úo encontrado para {personagem} - {estado_emocional}\nCaminho: {video_source}"}
            
        # Salva o ├âãÆ├é┬íudio temporariamente
        ...
        if duracao_audio is None or duracao_video_total is None:
            return {"success": False, "error": "FFprobe falhou ao ler m├âãÆ├é┬¡dia"}
            
        if duracao_audio > duracao_video_total:
            return {"success": False, "error": f"├âãÆ├é┬üudio ({duracao_audio:.1f}s) ├âãÆ├é┬® maior que o v├âãÆ├é┬¡deo ({duracao_video_total:.1f}s)"}
        ...
        # Limpa arquivo tempor├âãÆ├é┬írio
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            ...
            return {"success": False, "error": "V├âãÆ├é┬¡deo gerado est├âãÆ├é┬í vazio."}
...
async def volume_processar(
    ...
    try:
        ...
        # Salva os arquivos de m├âãÆ├é┬║sica se enviados
        ...
        for idx, video_file in enumerate(videos):
            ...
            if res.returncode == 0 and os.path.exists(output_path):
                ...
                erros.append(f"Erro no v├âãÆ├é┬¡deo {video_file.filename}: {res.stderr}")
                
        # Limpar m├âãÆ├é┬║sicas tempor├âãÆ├é┬írias
        for m_path, _ in musicas_selecionadas:
...
async def ferramentas_processar(
    ...
    try:
        ...
        if acao == "adicionar_capa":
            if not capa: raise Exception("Capa n├âãÆ├é┬úo enviada")
            ...
            # Pega dimens├âãÆ├é┬Áes do v├âãÆ├é┬¡deo
            ...
            if not largura or not altura: raise Exception("Largura e Altura s├âãÆ├é┬úo obrigat├âãÆ├é┬│rios")
            ...
            if not velocidade: raise Exception("Velocidade obrigat├âãÆ├é┬│ria")
            ...
            if brilho is None or contraste is None: raise Exception("Brilho e contraste obrigat├âãÆ├é┬│rios")
            ...
            if not rotacao: raise Exception("Rota├âãÆ├é┬º├âãÆ├é┬úo obrigat├âãÆ├é┬│ria")
            ...
            raise Exception("A├âãÆ├é┬º├âãÆ├é┬úo desconhecida")
        ...
        if os.path.exists(output_path):
            ...
            raise Exception("Arquivo de sa├âãÆ├é┬¡da n├âãÆ├é┬úo gerado.")
...
async def get_dashboard_stats():
    try:
        ...
        # 2. Hist├âãÆ├é┬│rico Local (Canal Atual)
...
async def load_profile(nome: str):
    ...
    if os.path.exists(path):
        with open(path, "r", encoding='utf-8') as f:
            return json.load(f)
    return JSONResponse(status_code=404, content={"message": "Perfil n├âãÆ├é┬úo encontrado"})

# ====== ENDPOINTS DA ABA TTS ======

    # Por enquanto, retornamos um dummy de sucesso
    print(f"[TTS] Gerando ├âãÆ├é┬íudio para: {req.voz} usando engine {req.engine}")
    return {"status": "success", "message": "├âãÆ├é┬üudio gerado com sucesso! (Simula├âãÆ├é┬º├âãÆ├é┬úo)", "path": ""}
        ...
        if srt_path:
            ...
            if engine == 'whisper':
                ...
                if not os.path.isdir(target):
                    raise Exception("Modelo Vosk n├âãÆ├é┬úo encontrado no caminho padr├âãÆ├é┬úo.")
...
async def api_dublagem_diarizar(video: UploadFile = File(...)):
    ...
    try:
        ...
        if not hf_token:
            return {"success": False, "error": "Token do HuggingFace n├âãÆ├é┬úo configurado."}
...
async def api_dublagem_processar(
    ...
    def worker():
        try:
            ...
            q.put(f"├â┬░├é┼©├é┼í├éÔé¼ Iniciando Processamento em Lote...\n")
            ...
            for item in configs_list:
                ...
                q.put(f"\n======================================")
                q.put(f"├â┬░├é┼©├é┼¢├é┬¼ [CENA {cena_idx}] Processando: {item['filename']}")
                ...
                try:
                    if item.get("is_multi") and item.get("diarization_map"):
                        q.put(f"├â┬░├é┼©├é┼¢├éÔäó├â┬»├é┬©├é┬Å MODO MULTI-VOZ DE EST├âãÆ├é┼íDIO ATIVADO")
                        ...
                        q.put("├â┬░├é┼©├éÔÇØ├éÔÇ× Aplicando Restaura├âãÆ├é┬º├âãÆ├é┬úo de ├âãÆ├é┬üudio...")
                        ap = AudioProcessor()
                        ...
                        if not char_config or not char_config.get("modelo_rvc"):
                            q.put(f"├â┬ó├é┬Ø├é┼Æ Personagem '{char_name}' sem modelo RVC.")
                            cena_idx += 1
                            continue
                        ...
                        q.put("├â┬░├é┼©├éÔÇØ├éÔÇ× Aplicando Restaura├âãÆ├é┬º├âãÆ├é┬úo de ├âãÆ├é┬üudio na voz isolada...")
                        ap = AudioProcessor()
                        ap.run_pipeline(rvc_audio, rvc_audio, cm.config)
                        
                        if b_skip_demucs or not bg_path:
                            q.put("Substituindo ├âãÆ├é┬íudio seco...")
                            import subprocess
                    ...
                    q.put(f"├â┬ó├é┼ô├éÔÇª Cena {cena_idx} conclu├âãÆ├é┬¡da! Salva como {os.path.basename(final_output)}")
                except Exception as e:
                    q.put(f"├â┬ó├é┬Ø├é┼Æ Erro na cena {cena_idx}: {str(e)}")
                    import traceback
                    traceback.print_exc()
            ...
            q.put(f"\n======================================")
            q.put(f"├â┬ó├é┼ô├éÔÇª PROCESSAMENTO EM LOTE CONCLU├âãÆ├é┬ìDO!")
        except Exception as e:
            q.put(f"\n├â┬ó├é┬Ø├é┼Æ Erro fatal: {str(e)}")
        finally:
            q.put(None)
...
# ===== ROTAS DO TANQUE DE COMBUST├âãÆ├é┬ìVEL =====

@app.get("/api/tanque/lista")
async def tanque_lista():
    if not CURRENT_WORKSPACE_PATH:
        return {"success": False, "error": "Workspace n├âãÆ├é┬úo configurado"}
        ...
        for f in os.listdir(pasta):
            if f.lower().endswith(tipos_validos):
                ...
                if os.path.isfile(caminho_abs):
                    ...
                    tipo = "V├âãÆ├é┬¡deo" if ext in ['.mp4', '.mkv', '.avi', '.mov'] else "├âãÆ├é┬üudio" if ext in ['.mp3', '.wav', '.m4a'] else "Imagem" if ext in ['.png', '.jpg', '.jpeg'] else "Template"
...
# ===== ROTAS DE GEST├âãÆ├éãÆO DE PERFIS (CENTRAL DE CONFIGURA├âãÆ├éÔÇí├âãÆ├éÔÇóES) =====
...
async def rename_config_profile(req: RenameProfileReq):
    try:
        ...
        if req.old_name not in perfis:
            return {"status": "error", "message": "Perfil antigo n├âãÆ├é┬úo encontrado."}
        if req.new_name in perfis:
            return {"status": "error", "message": "J├âãÆ├é┬í existe um perfil com o novo nome."}
...
async def rename_studio_profile(req: ManageStudioProfileReq):
    try:
        ...
        if not os.path.exists(old_path):
            return {"status": "error", "message": "Template n├âãÆ├é┬úo encontrado."}
        if os.path.exists(new_path):
            return {"status": "error", "message": "J├âãÆ├é┬í existe um template com o novo nome."}
        os.rename(old_path, new_path)
        return {"status": "success"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
# Monta arquivos tempor├âãÆ├é┬írios (samples de audio) e os est├âãÆ├é┬íticos NO FINAL para n├âãÆ├é┬úo sobrescrever rotas
temp_files_dir = os.path.join(os.getcwd(), 'temp')
os.makedirs(temp_files_dir, exist_ok=True)
app.mount("/temp", StaticFiles(directory=temp_files_dir), name="temp")

# Todo o conte├âãÆ├é┬║do de web_ui ser├âãÆ├é┬í servido estaticamente pelo app.mount no final do arquivo
# ========================================================
# FILA AUT├âãÆ├éÔÇØNOMA (BATCH QUEUE)
# ========================================================
...
class _ExternalManager:
    """Gerencia a execu├âãÆ├é┬º├âãÆ├é┬úo de programas externos (Node.js/React, etc) em background."""
    def __init__(self):
    ...
    def start_app(self, app_name):
        ...
        if not os.path.exists(app_dir):
            ...
            if not os.path.exists(app_dir):
                return {"status": "error", "message": f"Pasta do app n├âãÆ├é┬úo encontrada em Programas externos nem em FERRAMENTAS."}
...
class _FilaManager:
    """Gerenciador de fila de renderiza├âãÆ├é┬º├âãÆ├é┬úo aut├âãÆ├é┬┤noma em mem├âãÆ├é┬│ria."""
    STATUS_PENDENTE   = "├â┬ó├é┬Å├é┬│ Pendente"
    STATUS_RENDERANDO = "├â┬░├é┼©├éÔÇØ├éÔÇ× Renderizando..."
    STATUS_CONCLUIDO  = "├â┬ó├é┼ô├éÔÇª Conclu├âãÆ├é┬¡do"
    STATUS_ERRO       = "├â┬ó├é┬Ø├é┼Æ Erro"
    STATUS_CANCELADO  = "├â┬░├é┼©├é┼í├é┬½ Cancelado"

    def __init__(self):
        self.fila = []          # list of dicts
        self.rodando = False
        self.cancelar_flag = False
        self.log_buffer = []    # ├âãÆ├é┬║ltimas 500 linhas
    ...
    def adicionar(self, projeto: dict):
        with self._lock:
            ...
            projeto.setdefault("duracao", "├â┬ó├éÔé¼├éÔÇØ")
    ...
    def iniciar(self, callback_render=None):
        ...
        if self.rodando:
            return False, "A fila j├âãÆ├é┬í est├âãÆ├é┬í em execu├âãÆ├é┬º├âãÆ├é┬úo."
        self.cancelar_flag = False
        self.rodando = True
    ...
    def parar(self):
        ...
        self._log("├â┬ó├é┬Å├é┬© Sinal de parada enviado. Aguardando projeto atual...")        
    ...
    def _worker(self, callback_render=None):
        ...
        self._log(f"├â┬░├é┼©├é┼í├éÔé¼ Iniciando fila com {pendentes_total} projeto(s) pendente(s).")
        ...
        for proj in self.fila:
            ...
            self._log(f"├â┬░├é┼©├é┼¢├é┬¼ Iniciando: {proj['nome']}")
            ...
            try:
                if callback_render:
                    ...
                    # Simula├âãÆ├é┬º├âãÆ├é┬úo quando n├âãÆ├é┬úo h├âãÆ├é┬í render real conectado
                    _time.sleep(2)
                    ok = True
            except Exception as e:
                self._log(f"├â┬ó├é┬Ø├é┼Æ Exce├âãÆ├é┬º├âãÆ├é┬úo no render de '{proj['nome']}': {e}")
                ok = False
            ...
            if self.cancelar_flag:
                ...
                self._log(f"├â┬░├é┼©├é┼í├é┬½ '{proj['nome']}' cancelado.")
            elif ok:
                proj["status"] = self.STATUS_CONCLUIDO
                processados += 1
                self._log(f"├â┬ó├é┼ô├éÔÇª '{proj['nome']}' conclu├âãÆ├é┬¡do em {proj['duracao']}.")
            else:
                proj["status"] = self.STATUS_ERRO
                erros += 1
                self._log(f"├â┬ó├é┬Ø├é┼Æ '{proj['nome']}' falhou.")
        ...
        self._log(f"├â┬░├é┼©├é┬Å├é┬ü Fila finalizada em {total_sec:.0f}s. ├â┬ó├é┼ô├éÔÇª {processados} sucesso(s) | ├â┬ó├é┬Ø├é┼Æ {erros} erro(s).")
        self.rodando = False
        self.progresso = 100
        self.eta = "Conclu├âãÆ├é┬¡do"

    def toggle_hotfolder(self, pasta: str, ativo: bool):
        self.hotfolder_ativo = ativo
        self.hotfolder_pasta = pasta if ativo else ""
        if ativo:
            self._log(f"├â┬░├é┼©├é┬ñ├éÔÇô Piloto Autom├âãÆ├é┬ítico ATIVO ├â┬ó├éÔé¼├éÔÇØ monitorando: {pasta}")
            self._hotfolder_thread = _threading.Thread(
                target=self._hotfolder_worker, daemon=True)
            self._hotfolder_thread.start()
        else:
            self._log("├â┬░├é┼©├é┬ñ├éÔÇô Piloto Autom├âãÆ├é┬ítico DESATIVADO.")

    def _hotfolder_worker(self):
        """Monitora a pasta e enfileira novos arquivos de ├âãÆ├é┬íudio."""
...
async def fila_adicionar(req: FilaAdicionarReq):
    check_user_status()
    if not req.audio:
        return {"success": False, "error": "Campo 'audio' ├âãÆ├é┬® obrigat├âãÆ├é┬│rio."}
...
# ========================================================
# MONTADOR ├â┬ó├éÔé¼├éÔÇØ TRANSI├âãÆ├éÔÇí├âãÆ├éÔÇóES, CTAs E LOGOMARCAS
# ========================================================
...
async def montador_verificar_pastas(
    ...
    labels = {
        "pasta_trans_h": "Transi├âãÆ├é┬º├âãÆ├é┬Áes Horizontal",
        "pasta_trans_v": "Transi├âãÆ├é┬º├âãÆ├é┬Áes Vertical",
...
async def montador_processar(req: MontadorProcessarReq):
    """Processa v├âãÆ├é┬¡deos adicionando transi├âãÆ├é┬º├âãÆ├é┬Áes/CTAs/logomarcas. Retorna log por SSE."""
    check_user_status()
    import queue as _q
    import asyncio
    from fastapi.responses import StreamingResponse
    ...
    def _anexar_video_aleatorio(video_path, pasta_assets, sufixo, encoder):
        try:
            ...
            if not assets:
                _log(f"├â┬ó├é┼í├é┬á├â┬»├é┬©├é┬Å Nenhum .mp4 encontrado em: {pasta_assets}")
                return None
            asset_escolhido = _random.choice(assets)
            asset_path = os.path.join(pasta_assets, asset_escolhido)
            _log(f"├â┬░├é┼©├éÔÇ£├é┼¢ Usando asset: {asset_escolhido} ({sufixo})")
            ...
            if not os.path.exists(asset_temp):
                _log(f"├â┬ó├é┬Ø├é┼Æ Falha no redimensionamento do asset.")
                return None
            ...
            if res.returncode == 0 and os.path.exists(output_path):
                ...
                _log(f"├â┬ó├é┬Ø├é┼Æ Erro ffmpeg ({sufixo}): {res.stderr[-300:]}")
                return None
        except Exception as e:
            _log(f"├â┬ó├é┬Ø├é┼Æ Exce├âãÆ├é┬º├âãÆ├é┬úo ({sufixo}): {e}")
            return None
    ...
    def worker():
        ...
        try:
            ...
            try:
                ...
                encoder = 'libx264'
            _log(f"├â┬░├é┼©├éÔÇØ├é┬º Encoder detectado: {encoder}")
            ...
            _log(f"├â┬░├é┼©├é┼í├éÔé¼ Iniciando processamento de {total} v├âãÆ├é┬¡deo(s) ├â┬ó├éÔé¼├éÔÇØ Formato: {formato}")

            for idx, video_path in enumerate(req.videos):
                if not os.path.exists(video_path):
                    _log(f"├â┬ó├é┬Ø├é┼Æ Arquivo n├âãÆ├é┬úo encontrado: {video_path}")
                    _montador_status["erros"] += 1
                    continue

                nome = os.path.basename(video_path)
                _log(f"\n├â┬░├é┼©├éÔÇ£├é┬╣ [{idx+1}/{total}] Processando: {nome}")
                ...
                if req.usar_transicao and pasta_trans and os.path.exists(pasta_trans):
                    _log("  ├â┬ó├é┼¥├é┼ô Aplicando Transi├âãÆ├é┬º├âãÆ├é┬úo...")
                    ...
                    if resultado:
                        ...
                        _log("  ├â┬ó├é┼í├é┬á├â┬»├é┬©├é┬Å Transi├âãÆ├é┬º├âãÆ├é┬úo falhou, continuando sem ela.")

                if req.usar_cta and pasta_cta and os.path.exists(pasta_cta):
                    _log("  ├â┬ó├é┼¥├é┼ô Aplicando CTA...")
                    ...
                    if resultado:
                        ...
                        _log("  ├â┬ó├é┼í├é┬á├â┬»├é┬©├é┬Å CTA falhou, continuando.")

                if req.usar_logomarca and pasta_logo and os.path.exists(pasta_logo):
                    _log("  ├â┬ó├é┼¥├é┼ô Aplicando Logomarca+CTA...")
                    ...
                    if resultado:
                        ...
                        _log("  ├â┬ó├é┼í├é┬á├â┬»├é┬©├é┬Å Logomarca falhou, continuando.")
                ...
                if current != video_path:
                    ...
                    if current != final_name:
                        try:
                            ...
                            final_name = current
                    _log(f"  ├â┬ó├é┼ô├éÔÇª Salvo: {os.path.basename(final_name)}")
            ...
            _log(f"\n├â┬░├é┼©├é┬Å├é┬ü Conclu├âãÆ├é┬¡do! {_montador_status['processados']}/{total} v├âãÆ├é┬¡deo(s) processado(s).")
            _montador_status["mensagem"] = "Conclu├âãÆ├é┬¡do!"
        except Exception as e:
            _log(f"├â┬ó├é┬Ø├é┼Æ Erro fatal: {e}")
        finally:
...
async def laplata_generate(req: Request):
    ...
    try:
        ...
        prompt = ""
        system_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® uma Intelig├âãÆ├é┬¬ncia Artificial avan├âãÆ├é┬ºada. Responda em Portugu├âãÆ├é┬¬s do Brasil de forma direta, sem introdu├âãÆ├é┬º├âãÆ├é┬Áes desnecess├âãÆ├é┬írias."
        ...
        if tool == "prompt":
            ...
            prompt = f"Crie um prompt profissional (em ingl├âãÆ├é┬¬s, pois geradores de imagem funcionam melhor em ingl├âãÆ├é┬¬s) para gera├âãÆ├é┬º├âãÆ├é┬úo de imagens com IA baseado nos seguintes par├âãÆ├é┬ómetros:\nDescri├âãÆ├é┬º├âãÆ├é┬úo Principal: {desc}\nEstilo Visual: {estilo}\nQualidade: {qualidade}\nElementos Adicionais: {elementos}\nCrie um prompt detalhado e profissional."
            
        elif tool == "hashtag":
            niche = data.get("niche", "")
            market = data.get("market", "")
            quantity = data.get("quantity", "15")
            prompt = f"Crie {quantity} hashtags estrat├âãÆ├é┬®gicas para redes sociais baseado no nicho '{niche}' e mercado '{market}'. Retorne apenas as hashtags separadas por espa├âãÆ├é┬ºo."
            
        elif tool == "descrever":
            imagem_url = data.get("imageUrl", "")
            if not imagem_url:
                prompt = f"Descreva de forma extremamente detalhada e criativa a seguinte cena: {data.get('texto', 'uma cena gen├âãÆ├é┬®rica')} (Simulando uma imagem se a URL falhar)."
            else:
                prompt = f"Descreva a imagem contida nesta URL de forma rica em detalhes: {imagem_url}"
                
        elif tool == "mineracao":
            assunto = data.get("subject", "")
            prompt = f"Aja como um analista de tend├âãÆ├é┬¬ncias e algoritmo de redes sociais (TikTok, Reels, Shorts). O usu├âãÆ├é┬írio forneceu o nicho/assunto: '{assunto}'. Identifique os formatos e temas de v├âãÆ├é┬¡deos que est├âãÆ├é┬úo mais bombando (virais) atualmente neste nicho na internet. Forne├âãÆ├é┬ºa uma lista de 5 estruturas exatas de v├âãÆ├é┬¡deos virais, detalhando para cada um:\n1) O gancho (hook) visual e falado dos primeiros 3 segundos;\n2) O conceito/roteiro principal;\n3) Por que o algoritmo est├âãÆ├é┬í entregando tanto esse tipo de conte├âãÆ├é┬║do. Seja altamente estrat├âãÆ├é┬®gico."
            
        elif tool == "titulos":
            tema = data.get("topic", "")
            tipo = data.get("type", "youtube")
            prompt = f"Gere 10 t├âãÆ├é┬¡tulos altamente magn├âãÆ├é┬®ticos e com foco em clique (clickbait ├âãÆ├é┬®tico) para um conte├âãÆ├é┬║do de {tipo} sobre o assunto: '{tema}'."
            
        elif tool == "descricoes":
            titulo = data.get("title", "")
            palavras = data.get("keywords", "")
            prompt = f"Crie uma descri├âãÆ├é┬º├âãÆ├é┬úo para um v├âãÆ├é┬¡deo do YouTube com o t├âãÆ├é┬¡tulo '{titulo}'. A descri├âãÆ├é┬º├âãÆ├é┬úo deve ser persuasiva, incluir call to action (CTA) e ser otimizada para SEO utilizando estas palavras-chave: {palavras}."
            
        elif tool == "gerador_imagem":
            ideia = data.get("idea", "")
            prompt = f"Escreva 3 prompts de imagem distintos (em ingl├âãÆ├é┬¬s) otimizados para Midjourney/Stable Diffusion com base nesta ideia: '{ideia}'. Cada prompt deve ter um estilo art├âãÆ├é┬¡stico diferente (ex: realista, cyberpunk, anime)."
            
        elif tool == "tendencias":
            setor = data.get("sector", "")
            prompt = f"Liste as 5 principais tend├âãÆ├é┬¬ncias atuais (trends) para criadores de conte├âãÆ├é┬║do no setor de '{setor}'. Explique brevemente como um criador pode aproveitar cada tend├âãÆ├é┬¬ncia."
            
        if not prompt:
            return JSONResponse({"success": False, "error": "Ferramenta n├âãÆ├é┬úo suportada ou par├âãÆ├é┬ómetros inv├âãÆ├é┬ílidos."})
        ...
        if response.status_code == 200:
            ...
            # Ferramentas La Plata agora s├âãÆ├é┬úo gratuitas (custo 0) para incentivar o uso.
            # try:
            #     import user_database
...
# --- ENDPOINTS: CENTRAL DAS NOT├âãÆ├é┬ìCIAS (SIMBIOSE) ---
@app.post("/api/grok")
            ...
            for item in items:
                if 'videoRenderer' in item:
                    ...
                    try:
                        ...
                        views = 0
                        # Parse views like "1.2M views", "3,4 mil visualiza├âãÆ├é┬º├âãÆ├é┬Áes", "123 visualiza├âãÆ├é┬º├âãÆ├é┬Áes"
...
async def api_noticias_ai(req: NoticiasReq):
    ...
    if req.prompt_type == 'cacar':
        ...
        sys_prompt = f"""Voc├âãÆ├é┬¬ ├âãÆ├é┬® um jornalista investigativo e curador de not├âãÆ├é┬¡cias focado em pol├âãÆ├é┬¡tica e bastidores.
O usu├âãÆ├é┬írio quer saber as {count} not├âãÆ├é┬¡cias MAIS QUENTES e RECENTES sobre o tema fornecido.

MUITO IMPORTANTE SOBRE A BUSCA E AS FONTES:
- Use a ferramenta de busca combinando palavras-chave espec├âãÆ├é┬¡ficas para refinar a relev├âãÆ├é┬óncia.
- Exemplo de busca: "(Revista Oeste OR Gazeta do Povo OR Jovem Pan OR Twitter OR Pleno News OR Poder360)"
- N├âãÆ├éãÆO se limite apenas ├âãÆ├é┬á grande m├âãÆ├é┬¡dia tradicional (G1, CNN, Folha, UOL). Evite-as se poss├âãÆ├é┬¡vel.
- D├âãÆ├é┬¬ FORTE PREFER├âãÆ├é┼áNCIA e priorize m├âãÆ├é┬¡dias independentes, portais com vi├âãÆ├é┬®s de direita/conservador e jornalismo investigativo independente.
- Inclua tamb├âãÆ├é┬®m postagens, furos de reportagem ou debates relevantes do Twitter/X, se houver.
- O objetivo ├âãÆ├é┬® ter uma vis├âãÆ├é┬úo ampla que atenda a um p├âãÆ├é┬║blico que consome not├âãÆ├é┬¡cias de pol├âãÆ├é┬¡tica nacional, direita e bastidores de Bras├âãÆ├é┬¡lia.
- ORDENE SEMPRE DA MAIS RECENTE PARA A MAIS ANTIGA (focando em hoje e nos ├âãÆ├é┬║ltimos dias).

Retorne EXATAMENTE {count} not├âãÆ├é┬¡cias.
Para cada not├âãÆ├é┬¡cia, forne├âãÆ├é┬ºa:
1. title: O t├âãÆ├é┬¡tulo da not├âãÆ├é┬¡cia ou tweet.
2. summary: Um resumo bem curto (1 a 2 frases) do que se trata.
3. url: O link real da not├âãÆ├é┬¡cia ou da postagem.
4. source: O nome do portal, jornal ou rede social (ex: Revista Oeste, Twitter, Gazeta do Povo).
5. date: A data ou tempo de publica├âãÆ├é┬º├âãÆ├é┬úo (ex: "Hoje", "H├âãÆ├é┬í 2 horas", "Ontem").
6. imageUrl: A URL de uma imagem (thumbnail) da not├âãÆ├é┬¡cia. Se a busca retornar uma imagem associada ├âãÆ├é┬á mat├âãÆ├é┬®ria, coloque o link aqui. Se n├âãÆ├é┬úo encontrar, deixe vazio ("").
...
Responda APENAS com um array JSON com objetos contendo essas propriedades. NADA DE MARKDOWN."""
        user_prompt = f"Busque as {count} not├âãÆ├é┬¡cias mais recentes sobre: \"{req.input_text}\""
        

    elif req.prompt_type == 'analisar-canal':
        sys_prompt = f"""Atue como um estrategista de YouTube. Analise os seguintes v├âãÆ├é┬¡deos salvos pelo criador de conte├âãÆ├é┬║do. Com base nesses v├âãÆ├é┬¡deos, forne├âãÆ├é┬ºa:
1. Uma an├âãÆ├é┬ílise geral do nicho e do interesse do p├âãÆ├é┬║blico (quais temas geram mais interesse).
2. 5 ideias de v├âãÆ├é┬¡deos in├âãÆ├é┬®ditos inspirados nesse conte├âãÆ├é┬║do, mas com um ├âãÆ├é┬óngulo ├âãÆ├é┬║nico ou aprofundado.
3. Dicas de palavras-chave e estrat├âãÆ├é┬®gias de thumbnail para esse nicho.
Responda em Markdown, de forma clara e estruturada."""
        user_prompt = f"V├âãÆ├é┬¡deos salvos:\n{req.input_text}"


    elif req.prompt_type == 'monitorar-perfil':
        sys_prompt = f"""Extraia SOMENTE os dados reais e p├âãÆ├é┬║blicos referentes ├âãÆ├é┬á p├âãÆ├é┬ígina solicitada. Se n├âãÆ├é┬úo conseguir ler, retorne JSON com erro.
Responda ESTRITAMENTE em formato JSON com o seguinte schema:
{{
  "username": "Nome de usu├âãÆ├é┬írio",
  "followers": "N├âãÆ├é┬║mero de seguidores (ex: 1.2M)",
  "likes": "N├âãÆ├é┬║mero total de curtidas",
  "videos": "N├âãÆ├é┬║mero total de v├âãÆ├é┬¡deos",
  "recentVideos": [
    {{
      "title": "T├âãÆ├é┬¡tulo do v├âãÆ├é┬¡deo",
      "views": "100K",
      "likes": "5K",
      "date": "Data se houver"
    }}
        ...
        user_prompt = f"Dados extra├âãÆ├é┬¡dos da p├âãÆ├é┬ígina / URL:\n{req.input_text}"

    elif req.prompt_type == 'gerar-estrategia':
        sys_prompt = f"""Voc├âãÆ├é┬¬ ├âãÆ├é┬® um Estrategista Chefe de Conte├âãÆ├é┬║do para o YouTube e sabe tudo sobre tend├âãÆ├é┬¬ncias e algoritmos.
Sua miss├âãÆ├é┬úo ├âãÆ├é┬® criar um plano de ataque de conte├âãÆ├é┬║do URGENTE e PERSONALIZADO para o canal do usu├âãÆ├é┬írio, focado nas not├âãÆ├é┬¡cias mais quentes de hoje.
Gere 3 ideias de v├âãÆ├é┬¡deos ALTAMENTE ESTRAT├âãÆ├éÔÇ░GICOS que o usu├âãÆ├é┬írio deve gravar AGORA.

Para cada ideia, forne├âãÆ├é┬ºa:
- title: Um t├âãÆ├é┬¡tulo forte e magn├âãÆ├é┬®tico.
- whyNow: Por que este v├âãÆ├é┬¡deo deve ser feito HOJE (urg├âãÆ├é┬¬ncia, timing, hype).
- angle: Qual o ├âãÆ├é┬óngulo ├âãÆ├é┬║nico que o usu├âãÆ├é┬írio deve abordar para se diferenciar da concorr├âãÆ├é┬¬ncia.
- urgency: "Alta", "M├âãÆ├é┬®dia" ou "Baixa".
- competitorContext: O que a concorr├âãÆ├é┬¬ncia est├âãÆ├é┬í fazendo sobre isso e como o usu├âãÆ├é┬írio vai super├âãÆ├é┬í-los.

Responda APENAS com um array JSON. NADA DE MARKDOWN."""
        user_prompt = req.input_text
        
    elif req.prompt_type == 'gerar-roteiro':
        base_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® um roteirista de YouTube e especialista em SEO."
        if req.profile_context:
            base_prompt += f"\nINSTRU├âãÆ├éÔÇí├âãÆ├éÔÇóES DE IDENTIDADE DO CANAL:\nVoc├âãÆ├é┬¬ DEVE adotar estritamente a identidade, linguagem e estilo definidos nos seguintes documentos do canal:\n{req.profile_context}\n"
            
        if req.format == 'shorts':
            sys_prompt = f"{base_prompt}\nCrie um roteiro EXTREMAMENTE DIN├âãÆ├éÔÇÜMICO, R├âãÆ├é┬üPIDO E RETENTIVO (menos de 60 segundos).\nTom selecionado: {req.tone}. Adapte o texto para este tom.\n\nSua resposta DEVE seguir EXATAMENTE esta estrutura em Markdown:\n# ├â┬░├é┼©├éÔÇ£├é┬▒ Op├âãÆ├é┬º├âãÆ├é┬Áes de T├âãÆ├é┬¡tulo (Para a legenda do v├âãÆ├é┬¡deo)\n[3 op├âãÆ├é┬º├âãÆ├é┬Áes]\n# ├â┬░├é┼©├éÔÇ£├é┼ô Roteiro do V├âãÆ├é┬¡deo Curto (TikTok/Shorts)\n[Roteiro com Hook, Corpo, CTA e Sugest├âãÆ├é┬Áes Visuais entre colchetes]"
        else:
            sys_prompt = f"{base_prompt}\nCrie um pacote completo (Roteiro + SEO) envolvente e direto considerando a modalidade: {req.format}.\nTom selecionado: {req.tone}. Adapte o texto para este tom.\n\nSua resposta DEVE seguir EXATAMENTE esta estrutura em Markdown:\n# ├â┬░├é┼©├é┼¢├é┬¼ Op├âãÆ├é┬º├âãÆ├é┬Áes de T├âãÆ├é┬¡tulo (Alta Taxa de Clique)\n[3 op├âãÆ├é┬º├âãÆ├é┬Áes]\n# ├â┬░├é┼©├éÔÇ£├é┬Ø Descri├âãÆ├é┬º├âãÆ├é┬úo Otimizada para SEO\n[Descri├âãÆ├é┬º├âãÆ├é┬úo com palavras-chave]\n# ├â┬░├é┼©├é┬Å├é┬À├â┬»├é┬©├é┬Å Tags Virais\n[15 a 20 tags]\n# ├â┬░├é┼©├éÔÇ£├é┼ô Roteiro do V├âãÆ├é┬¡deo\n[Roteiro completo: Gancho, Desenvolvimento, Sugest├âãÆ├é┬Áes Visuais e Conclus├âãÆ├é┬úo]"
        user_prompt = f"Use as seguintes informa├âãÆ├é┬º├âãÆ├é┬Áes/t├âãÆ├é┬│picos para criar o roteiro:\n\n{req.input_text}"
        
    elif req.prompt_type == 'minerar':
        sys_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® um estrategista de conte├âãÆ├é┬║do para YouTube.\nAnalise o v├âãÆ├é┬¡deo fornecido e sugira 3 ideias de v├âãÆ├é┬¡deos similares, POR├âãÆ├éÔÇ░M MELHORES, para eu gravar no meu canal.\nDesconstrua os gatilhos mentais do t├âãÆ├é┬¡tulo e sugira como o meu roteiro deve ser estruturado.\nUse formata├âãÆ├é┬º├âãÆ├é┬úo Markdown."
        user_prompt = req.input_text
        
    elif req.prompt_type == 'estrategia':
        sys_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® um estrategista de YouTube n├âãÆ├é┬¡vel S├âãÆ├é┬¬nior. Sua tarefa ├âãÆ├é┬® analisar o contexto do canal e a lista de concorrentes fornecida, e sugerir 5 ideias de v├âãÆ├é┬¡deos URGENTES ou EXCLUSIVAS que o canal deve gravar AGORA para se destacar.\n\nRetorne EXATAMENTE um JSON que ├âãÆ├é┬® um array de 5 objetos com as seguintes propriedades:\n- title: T├âãÆ├é┬¡tulo forte e chamativo do v├âãÆ├é┬¡deo.\n- urgency: Grau de urg├âãÆ├é┬¬ncia (Alta!, Alta, M├âãÆ├é┬®dia, Baixa) baseado em tend├âãÆ├é┬¬ncias atuais.\n- whyNow: Por que este v├âãÆ├é┬¡deo deve ser feito hoje (ex: hype, not├âãÆ├é┬¡cia de ├âãÆ├é┬║ltima hora, gap no mercado).\n- angle: Qual o ├âãÆ├é┬óngulo ├âãÆ├é┬║nico/diferencial que nosso canal pode adotar em rela├âãÆ├é┬º├âãÆ├é┬úo ├âãÆ├é┬á concorr├âãÆ├é┬¬ncia.\n- competitorContext: Como os concorrentes cobriram (ou falharam em cobrir) este assunto.\n\nResponda APENAS com o JSON v├âãÆ├é┬ílido, sem formato markdown (sem ```json)."
        user_prompt = req.input_text

    elif req.prompt_type == 'dashboard':
        sys_prompt = f"Retorne as 6 not├âãÆ├é┬¡cias mais importantes do dia sobre a categoria: {req.input_text}.\nResponda APENAS com um array JSON com objetos contendo: title, summary, source, date, url.\nSem formatacao markdown, apenas o json cru."
        user_prompt = "Me d├âãÆ├é┬¬ as not├âãÆ├é┬¡cias."
        
    elif req.prompt_type == 'images':
        sys_prompt = f"Voc├âãÆ├é┬¬ ├âãÆ├é┬® um diretor de arte experiente e precisa selecionar as melhores imagens de B-roll para um v├âãÆ├é┬¡deo do YouTube. O usu├âãÆ├é┬írio fornecer├âãÆ├é┬í um script e o n├âãÆ├é┬║mero de imagens ({req.images_count}) que precisam ser selecionadas. Retorne EXATAMENTE UM JSON ARRAY com {req.images_count} objetos e NADA MAIS. NADA DE MARKDOWN.\n\nFormato obrigat├âãÆ├é┬│rio do JSON:\n[{{\n  \"id\": 1,\n  \"description\": \"Descri├âãÆ├é┬º├âãÆ├é┬úo do que precisamos ver na imagem (em PT-BR)\",\n  \"searchQuery\": \"Uma query EM INGL├âãÆ├é┼áS pronta para ser buscada no Google Images\",\n  \"context\": \"O trecho do script que motivou esta imagem\",\n  \"type\": \"photo\" // Um dos tipos permitidos\n}}]\n\nTipos permitidos: {', '.join(req.allowed_types)}."
        user_prompt = f"Extraia {req.images_count} imagens de apoio para este roteiro:\n\n{req.input_text}"
        
    elif req.prompt_type == 'seo':
        sys_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® um especialista em SEO para YouTube (foco em v├âãÆ├é┬¡deos virais e reten├âãÆ├é┬º├âãÆ├é┬úo). Seu objetivo ├âãÆ├é┬® otimizar t├âãÆ├é┬¡tulos, descri├âãÆ├é┬º├âãÆ├é┬Áes e tags para v├âãÆ├é┬¡deos. Retorne o resultado em formato de texto estruturado."
        user_prompt = f"Otimize o SEO para o seguinte roteiro de v├âãÆ├é┬¡deo:\n\n{req.input_text}"
        

    elif req.prompt_type == 'gerar-shorts':
        sys_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® um roteirista de YouTube Shorts focado em reten├âãÆ├é┬º├âãÆ├é┬úo. Crie um roteiro EXTREMAMENTE DIN├âãÆ├éÔÇÜMICO e R├âãÆ├é┬üPIDO (menos de 60 segundos) com base na not├âãÆ├é┬¡cia/texto fornecido. Retorne em formato Markdown com as se├âãÆ├é┬º├âãÆ├é┬Áes: # ├â┬░├é┼©├éÔÇ£├é┬▒ Op├âãÆ├é┬º├âãÆ├é┬Áes de T├âãÆ├é┬¡tulo (3 op├âãÆ├é┬º├âãÆ├é┬Áes), # ├â┬░├é┼©├éÔÇ£├é┼ô Roteiro do V├âãÆ├é┬¡deo Curto."
        user_prompt = f"Gere um roteiro curto para: {req.input_text}"

    elif req.prompt_type == 'ideias-thumbnails':
        sys_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® um designer de thumbnails de YouTube focado em CTR (Click-Through Rate). Analise o roteiro ou contexto fornecido e sugira 3 ideias brilhantes de thumbnails que despertem alta curiosidade. Descreva os elementos visuais, a composi├âãÆ├é┬º├âãÆ├é┬úo, e o texto exato na imagem para cada uma. Retorne em formato Markdown."
        user_prompt = f"Sugira 3 thumbnails para:\n\n{req.input_text}"

    elif req.prompt_type == 'deep-dive':
        sys_prompt = "Voc├âãÆ├é┬¬ ├âãÆ├é┬® um jornalista investigativo e analista geopol├âãÆ├é┬¡tico/econ├âãÆ├é┬┤mico. Aprofunde-se no contexto da not├âãÆ├é┬¡cia fornecida, explicando o pano de fundo, os interesses envolvidos, os desdobramentos futuros e o que a grande m├âãÆ├é┬¡dia pode estar omitindo. Retorne em formato Markdown."
...
@app.api_route("/api/studio/modal/{path:path}", methods=["GET", "POST", "OPTIONS"])
async def modal_proxy(path: str, request: Request):
    ...
    print(f"[Modal Proxy] >>> Endpoint: {path} | M├â┬®todo: {request.method}")
    print(f"[Modal Proxy] Contas modais v├â┬ílidas encontradas: {len(valid_modal_accounts)}")
    ...
    if not valid_modal_accounts:
        ...
        print(f"[Modal Proxy] CRITICO: Sem contas v├â┬ílidas! Contas modal total={len(all_modal)}")
        for acc in all_modal:
            print(f"  - {acc.get('name')}: is_active={acc.get('is_active')}, balance={acc.get('last_balance')}")
        return Response(status_code=503, content="CR├â┬ìTICO: Nenhuma conta Cloud Modal com saldo dispon├â┬¡vel.")

    random.shuffle(valid_modal_accounts)
    acc = valid_modal_accounts[0]
    workspace = acc.get('workspace')

    # Roteamento centralizado da aplica├â┬º├â┬úo ASGI Modal
    endpoint_path = path
    ...
    if request.method == "GET":
        ...
        # INTERCEPTA├âÔÇí├âãÆO MULTI-PASS
        if path == "generate_image":
            ...
            try:
                ...
                if ref_images and len(ref_images) >= 2 and body_json.get("model", "").lower() == "flux2-universal":
                    print(f"[Modal Proxy] Interceptado: {len(ref_images)} imagens de refer├â┬¬ncia detectadas. Iniciando Multipass Workflow.")
                    from backend.api.ai_director_multipass import AIDirectorMultipass
                    import asyncio
                    
                    async def multipass_generator():
                        yield json.dumps({"status": "processing", "message": "Iniciando intelig├â┬¬ncia de dire├â┬º├â┬úo de arte (LLM)..."}).encode('utf-8') + b"\n"
                        try:
                            import os
                            ...
                            yield json.dumps({"status": "processing", "message": f"Planejando cen├â┬írio base e {len(ref_images)} personagens com LLM..."}).encode('utf-8') + b"\n"
                            
                            # Precisamos rodar isso em um executor porque o Director ├â┬® bloqueante (requests.post)
                            def run_sync_director():
                                # Passo 1: Planejamento
                                base_prompt, regional_prompts = director.break_down_prompt(global_prompt, characters)
                                # Passo 2: Gera├â┬º├â┬úo Base
        ...
        if path == "generate/audio_lab":
            ...
            async def audio_interceptor():
                yield json.dumps({"status": "processing", "message": "Iniciando gera├º├úo de ├íudio no Modal..."}).encode('utf-8') + b'\n'
                try:
                    print(f"[Modal Proxy] Interceptando requisi├º├úo para audio_lab...")
                    import httpx
                    import asyncio
                    async with httpx.AsyncClient(timeout=1200.0) as local_client:
                        ...
                        while not task.done():
                            yield json.dumps({"status": "processing", "message": "Processando ├íudio na nuvem..."}).encode('utf-8') + b'\n'
    ...
    async def stream_generator():
        response = None
        try:
            print(f"[Modal Proxy] Abrindo conex├â┬úo com Modal...")
            response = await client.send(req, stream=True)
            elapsed_connect = time.time() - t_start
            print(f"[Modal Proxy] Conex├â┬úo estabelecida em {elapsed_connect:.1f}s | HTTP {response.status_code}")
            if response.status_code != 200:
...
async def studio_generate(req: StudioGenerateRequest, background_tasks: BackgroundTasks):
    ...
    # Envia a tarefa pesada para o background (para n├â┬úo bloquear o request)
    background_tasks.add_task(worker_studio_generate, job_id, payload)
    
    return {"success": True, "job_id": job_id, "status": "QUEUED", "message": "Adicionado ├â┬á fila de renderiza├â┬º├â┬úo."}
...
async def enhance_prompt(req: EnhancePromptRequest):
    ...
    # Fallback caso n├â┬úo tenha API
    if req.type == 'image':
...
async def enhance_video_api(req: EnhanceVideoRequest, background_tasks: BackgroundTasks):
    ...
    if not row or row[0] != 'video_generation' or not row[1]:
        return {"success": False, "error": "V├â┬¡deo original inv├â┬ílido ou inexistente."}
...
async def generate_sfx_api(req: GenerateSFXRequest, background_tasks: BackgroundTasks):
    ...
    async def worker_sfx(j_id, prompt):
        ...
        try:
            ...
            # Executa de forma s├â┬¡ncrona dentro da task do FastAPI
...
async def public_explore():
    # Retorna os ├â┬║ltimos 50 jobs completos (simulando um feed p├â┬║blico)
    conn = get_db_connection()
    c = conn.cursor()
...
async def colmeia_logs():
    """L├¬ o log de execu├º├úo em tempo real para exibir no painel esquerdo."""
    log_path = "colmeia_execution.log"
    try:
        if not os.path.exists(log_path):
            return PlainTextResponse("Aguardando logs de execu├º├úo...")
        with open(log_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
            return PlainTextResponse("".join(lines[-100:]))
    except Exception as e:
        return PlainTextResponse(f"Erro ao ler logs de execu├º├úo: {e}", status_code=500)
...
async def ws_voice(websocket: WebSocket, channel: str = "default"):
    ...
    async def run_llm_and_tts(user_text):
        try:
            ...
            import httpx
            import base64
            print("[WS] Configurando conex├úo XTTS...")
            ...
            system_prompt = f"Voc├¬ ├® a IA de comunica├º├úo em tempo real do canal '{channel}' do Apollo Edit Web. Fale em portugu├¬s do brasil com um tom muito natural e humano. Responda SEMPRE de forma ultra curta e r├ípida, em no m├íximo 1 ou 2 frases curtas, para manter a conversa fluida e n├úo gastar tempo. {contexto}"
            
            # Carrega uma voz de refer├¬ncia padr├úo caso exista
            ref_bytes = b""
            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    # Envia em pequenos peda├ºos (simulando stream para o TTS)
                    import textwrap
            ...
            print(f"[WS] Gera├º├úo interrompida (Task Cancelada) no canal {channel}")
        except Exception as e:
            import traceback
            ...
            if "text" in message and message["text"] is not None:
                ...
                try:
                    ...
                    if data.get("type") == "voice_config":
                        ...
                        print(f"[WS Usu├írio Texto - Canal {channel}]: {text_val}")
                        if current_generation_task and not current_generation_task.done():
                            current_generation_task.cancel()
                ...
                try:
                    ...
                    if text and text.strip():
                        print(f"[WS Usu├írio - Canal {channel}]: {text}")
                        if current_generation_task and not current_generation_task.done():
                            current_generation_task.cancel()
...
def worker_studio_generate(job_id: str, payload: dict):
    ...
    if not valid_modal_accounts:
        economy_db.update_job_status(job_id, 'FAILED', result_url="CR├â┬ìTICO: Nenhuma conta Cloud Modal dispon├â┬¡vel.")
        return
    ...
    for acc in valid_modal_accounts:
        ...
        print(f"[Gateway Job {job_id}] Roteando renderiza├â┬º├â┬úo para Modal Workspace: {workspace}")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"[Gateway Job {job_id}] Workspace {workspace} falhou com c├â┬│digo {r.status_code}. Zerando saldo...")
            ...
            last_error = "Timeout na requisi├â┬º├â┬úo."
        except Exception as e:
            last_error = str(e)
            
    # Se chegou aqui, todas falharam
    economy_db.update_job_status(job_id, 'FAILED', result_url=f"Falha em todas as contas. ├â┼íltimo erro: {last_error}")
    import asyncio
    if 'job_notifier' in globals():
...
# Montar diret├â┬│rios externos para apps est├â┬íticos
...
# Todo o conte├âãÆ├é┬║do de web_ui ser├âãÆ├é┬í servido estaticamente (Deve ser a ├âãÆ├à┬íLTIMA rota)
app.mount("/", StaticFiles(directory=WEB_UI_DIR), name="static")

def start_server(workspace_name, workspace_path, port=8080):
    """Fun├âãÆ├é┬º├âãÆ├é┬úo chamada pelo apollo_studio.py para rodar o uvicorn."""
    ...
    try:
        ...
        worker.start()
        print("Cloud Sync Worker (M├â┬│dulo C) iniciado com sucesso no background.")
    except Exception as e:
