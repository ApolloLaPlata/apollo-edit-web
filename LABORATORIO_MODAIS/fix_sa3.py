...
app = modal.App("apollo-engine-sa3")
...
class StableAudioEngine:
    ...
    def setup(self):
        ...
        # NOTE: Using environment variables or secure secrets in production
        login("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")
        
        print("[INIT] Baixando configs e pesos do Stable Audio 3 Medium...")
    ...
    def generate_audio(self, prompt: str, duration_s: float = 120.0, num_inference_steps: int = 8, seed: int = 0):
        ...
        print(f"[GEN] Gerando {duration_s}s no SA3 Medium | Prompt: {prompt[:50]}... | CFG: {cfg}, Steps: {steps}, Sampler: dpmpp-3m-sde")
        start = time.time()
        ...
        print(f"[GEN] Gerado em {time.time()-start:.1f} segundos!")
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)
print("SA3 re-escrito usando inpaint e 8 steps (igual ao test final)")
