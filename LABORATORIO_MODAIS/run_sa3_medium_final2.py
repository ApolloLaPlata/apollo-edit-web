...
app = modal.App("stable-audio-3-engine-final2")
...
class StableAudio3EngineFinal2:
    @modal.enter()
    def setup(self):
        import torch
    ...
    def generate(self, prompt: str, seconds: int = 120):
        ...
        print(f"Gerando {seconds}s no Stable Audio 3 Medium para: {prompt} (CFG: {cfg}, Steps: {steps}, Sampler: EULER)")
        start = time.time()
        ...
        with torch.no_grad():
            output = generate_diffusion_cond_inpaint(
                ...
                sampler_type="euler", # FIX ABSOLUTO: O UNICO SAMPLER SUPORTADO PARA RECTIFIED FLOW (SA3) 
                device="cuda"
            )
...
def run_tests():
    import base64
    os.makedirs("E:/MEUS PROGRAMAS/APOLLO_EDIT_WEB/LABORATORIO_MODAIS/testes_sa3_final", exist_ok=True)
    engine = StableAudio3EngineFinal2()
    
    tasks = [
        {
            "name": "Instrumental_Cinematico_Final",
    ...
    print(f"Enviando bateria de testes DEFINITIVA (Distilled) para o modelo *Stable Audio 3 Medium*...")
    ...
    for task in tasks:
        ...
        out_path = f"E:/MEUS PROGRAMAS/APOLLO_EDIT_WEB/LABORATORIO_MODAIS/testes_sa3_final/{task['name']}_{timestamp}.wav"
