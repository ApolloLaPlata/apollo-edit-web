...
document.getElementById('btn-confirm-export')?.addEventListener('click', () => {
    ...
    // GANHO DE KM (Sistema Crafter)
    if (window.parent) {
        window.parent.postMessage({ type: 'apollo_export_km', amount: 50 }, '*');
    }
...
async function sendChatMessage() {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            appendChatMessage('copilot', aiResponse.message);
    ...
    ops.forEach(op => {
        try {
            if (op.type === 'add_clip') {
                ...
                const leftClip = State.clips.find(c => String(c.id) === String(op.left_id));
                const rightClip = State.clips.find(c => String(c.id) === String(op.right_id));
                if (leftClip && rightClip && typeof createTransitionBlock === 'function') {
                    createTransitionBlock(leftClip, rightClip, op.transition_type || 'fade', op.duration || 1.0);
                }
            }
...
// --- TAB LIBRARY LOGIC (Bagageiro vs Assets) ---
window.switchLibraryTab = function(tabName) {
    ...
    document.getElementById('media-grid-bagageiro').style.display = tabName === 'bagageiro' ? 'grid' : 'none';
    document.getElementById('media-grid-assets').style.display = tabName === 'assets' ? 'grid' : 'none';
    
    if(tabName === 'bagageiro') {
        syncBagageiroItems();
    }
};

// Sincroniza visualmente os itens do Bagageiro (Pai) com a Timeline
window.syncBagageiroItems = function() {
    try {
        ...
        if (window.parent && window.parent.Visualizador) {
            ...
            if(parentBagageiro.length > 0) {
                ...
                parentBagageiro.forEach(item => {
                    ...
                    clone.dataset.type = clone.dataset.type || 'video';
                    clone.dataset.duration = '10.0'; // mock duration
                    clone.addEventListener('dragstart', (e) => {
                        e.dataTransfer.setData('text/plain', JSON.stringify({
                            type: clone.dataset.type,
                            duration: 10.0,
                            name: clone.textContent.trim(),
                            color: clone.dataset.type === 'video' ? '#3b82f6' : '#f97316'
                        }));
                        e.dataTransfer.effectAllowed = 'copy';
                    });
        ...
        console.log("Erro de cross-origin ou bagageiro vazio", e);
    }
};
